"""Local-only history-reader upgrade: recovery, ancestor retention and attestation."""
import copy
...
q = load('history_reader_ops', 'scripts/deploy_history_reader_20260914.py')
fixture = load('history_reader_transaction_fixture',
...
def predecessor_gc_adapter(adapter=None):
    ...
    cold_metadata_adapter = adapter.previous_adapter()
...
def candidate_engine():
    ...
    adapter = predecessor_adapter()
    engine = predecessor_gc_adapter(adapter).previous_module('/tmp/test-history-reader-engine.py')
...
class TransactionTests(fixture.TransactionTests):
    def setUp(self):
        ...
        self.ancestor_files = []
        for revision in ('D656', '8058', 'af989', '8a0681d3', '64ed654f'):
    ...
    def test_every_file_replace_and_reload_failure_restores_cda09766_on(self):
    ...
    def test_upgrade_and_manual_on_off_rollback_preserve_all_six_ancestors(self):
    ...
    def test_partial_transaction_reconstruction_can_restore_cda09766(self):
    ...
    def test_each_missing_candidate_queue_metric_restores_valid_cda09766(self):
        for key in self.b.queue_adapter.QUEUE_METRICS:
    ...
    def test_all_operator_signals_restore_handlers_and_cda09766(self):
...
class ModuleTests(unittest.TestCase):
    def make_dependencies(self):
        old_h = types.SimpleNamespace(RELEASE=Path('/cda09766'), BINARY=Path('/cda09766/gtron'))
        old_i = types.SimpleNamespace(CURRENT_SOURCE='64ed654f', RELEASE=Path('/cda09766/source'))
        ...
        record = {'binary_sha256': 'b' * 64, 'old_armed': marker64ed}
        self.checks = []
        ...
        old_bundle = types.SimpleNamespace(original=original, i=old_i, h=old_h,
            guard=fixture.s, g=fixture.g, ancestor_markers=(marker8a, markerAf989, marker8058, markerD656),
        ...
        def previous_load(args):
            ...
            return old_bundle
        previous = types.SimpleNamespace(BINARY=Path('/cda09766/gtron'), RELEASE=Path('/cda09766'),
        ...
        adapter = types.SimpleNamespace(configure=lambda engine: previous,
            previous_adapter=lambda: cold_metadata_adapter)
    ...
    def test_load_immutable_predecessor_does_not_compare_live_partial_transaction(self):
        ...
        self.assertEqual(builder.NATIVE_TEST_PATTERN,
            'Test.*(History|Shared|StateChange|StateDomainChangePrune|AsOf|Unwind|ResetMutableState|Snapshot.*Coverage|Manifest|Companion|Trusted|Delegation|Delegated|DrAccountIndex|Commitment)')
        self.assertIs(bundle.ancestor_markers[0], self.old_record['old_armed'])
        self.assertEqual(bundle.ancestor_markers[1]['path'], '/8a0681d3/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[2]['path'], '/af989/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[3]['path'], '/8058/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[4]['path'], '/d656/reader-required.json')
        self.assertEqual(len(bundle.ancestor_markers), 5)
        ...
        for unapproved in ('/b130/gtron', '/d656/gtron', '/8058/gtron', '/af989/gtron', '/8a0681d3/gtron', '/64ed654f/gtron'):
    ...
    def test_wrong_predecessor_prepared_sha_rejected_before_loading(self):
        ...
        with patch.object(q, 'previous_adapter') as loader:
            for bad in ('', q.PREDECESSOR_PARENT_SHA, '0' * 64):
                with self.subTest(sha=bad), self.assertRaisesRegex(RuntimeError, 'cda09766 predecessor'):
    ...
    def test_pinned_7b5d21b2_adapter_and_raw_engines_are_independent(self):
        ...
        self.assertEqual(old.CURRENT_SOURCE, '64ed654f582a58f8c03578ba38cd78f7ba4d385e')
        self.assertEqual(old.RELEASE, Path('/data/gtron/releases/20260914-large-history'))
    ...
    def test_main_resolves_raw_engine_through_all_four_pinned_adapters(self):
        ...
        with patch.object(q, 'APPROVED', True), \
                patch.object(q, 'previous_adapter', return_value=large_history) as load_adapter, \
    ...
    def test_frozen_scope_matches_commit_when_available(self):
        ...
        self.assertNotIn('scripts/tests/test_deploy_history_reader_20260914.py', actual)
