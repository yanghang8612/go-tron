"""Local-only commitment-prefetch upgrade: recovery, ancestor retention and attestation."""
import copy
...
q = load('commitment_prefetch_ops', 'scripts/deploy_commitment_prefetch_20260914.py')
fixture = load('commitment_prefetch_transaction_fixture',
...
def predecessor_gc_adapter(adapter=None):
    ...
    large_history_adapter = adapter.previous_adapter()
...
def candidate_engine():
    ...
    adapter = predecessor_adapter()
    engine = predecessor_gc_adapter(adapter).previous_module('/tmp/test-commitment-prefetch-engine.py')
...
class TransactionTests(fixture.TransactionTests):
    def setUp(self):
        ...
        self.ancestor_files = []
        for revision in ('D656', '8058', 'af989', '8a0681d3', '64ed654f', 'cda09766'):
    ...
    def test_every_file_replace_and_reload_failure_restores_833e4a0b_on(self):
    ...
    def test_upgrade_and_manual_on_off_rollback_preserve_all_seven_ancestors(self):
    ...
    def test_partial_transaction_reconstruction_can_restore_833e4a0b(self):
    ...
    def test_each_missing_candidate_queue_metric_restores_valid_833e4a0b(self):
        for key in self.b.queue_adapter.QUEUE_METRICS:
    ...
    def test_all_operator_signals_restore_handlers_and_833e4a0b(self):
...
class ModuleTests(unittest.TestCase):
    def make_dependencies(self):
        old_h = types.SimpleNamespace(RELEASE=Path('/833e4a0b'), BINARY=Path('/833e4a0b/gtron'))
        old_i = types.SimpleNamespace(CURRENT_SOURCE='cda09766', RELEASE=Path('/833e4a0b/source'))
        ...
        record = {'binary_sha256': 'b' * 64, 'old_armed': markerCda}
        self.checks = []
        ...
        old_bundle = types.SimpleNamespace(original=original, i=old_i, h=old_h,
            guard=fixture.s, g=fixture.g, ancestor_markers=(marker64ed, marker8a, markerAf989, marker8058, markerD656),
        ...
        def previous_load(args):
            ...
            return old_bundle
        previous = types.SimpleNamespace(BINARY=Path('/833e4a0b/gtron'), RELEASE=Path('/833e4a0b'),
        ...
        adapter = types.SimpleNamespace(configure=lambda engine: previous,
            previous_adapter=lambda: large_history_adapter)
    ...
    def test_load_immutable_predecessor_does_not_compare_live_partial_transaction(self):
        ...
        self.assertEqual(bundle.ancestor_markers[1]['path'], '/64ed654f/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[2]['path'], '/8a0681d3/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[3]['path'], '/af989/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[4]['path'], '/8058/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[5]['path'], '/d656/reader-required.json')
        self.assertEqual(len(bundle.ancestor_markers), 6)
        ...
        for unapproved in ('/b130/gtron', '/d656/gtron', '/8058/gtron', '/af989/gtron', '/8a0681d3/gtron', '/64ed654f/gtron', '/cda09766/gtron'):
    ...
    def test_wrong_predecessor_prepared_sha_rejected_before_loading(self):
        ...
        with patch.object(q, 'previous_adapter') as loader:
            for bad in ('', q.PREDECESSOR_PARENT_SHA, '0' * 64):
                with self.subTest(sha=bad), self.assertRaisesRegex(RuntimeError, '833e4a0b predecessor'):
    ...
    def test_pinned_4c1bc43b_adapter_and_raw_engines_are_independent(self):
        ...
        self.assertEqual(old.CURRENT_SOURCE, 'cda09766dfc6cce85253484430f4297d8751ea92')
        self.assertEqual(old.RELEASE, Path('/data/gtron/releases/20260914-history-reader'))
    ...
    def test_main_resolves_raw_engine_through_all_five_pinned_adapters(self):
        ...
        with patch.object(q, 'APPROVED', True), \
                patch.object(q, 'previous_adapter', return_value=history_reader) as load_adapter, \
    ...
    def test_frozen_scope_matches_commit_when_available(self):
        ...
        self.assertNotIn('scripts/tests/test_deploy_commitment_prefetch_20260914.py', actual)
