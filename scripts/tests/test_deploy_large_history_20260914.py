"""Local-only large-history upgrade: recovery, ancestor retention and attestation."""
import copy
...
q = load('large_history_ops', 'scripts/deploy_large_history_20260914.py')
fixture = load('large_history_transaction_fixture',
...
def predecessor_gc_adapter(adapter=None):
    ...
    companion_adapter = adapter.previous_adapter()
...
def candidate_engine():
    ...
    adapter = predecessor_adapter()
    engine = predecessor_gc_adapter(adapter).previous_module('/tmp/test-large-history-engine.py')
...
class TransactionTests(fixture.TransactionTests):
    def setUp(self):
        ...
        self.ancestor_files = []
        for revision in ('D656', '8058', 'af989', '8a0681d3'):
    ...
    def test_every_file_replace_and_reload_failure_restores_64ed654f_on(self):
    ...
    def test_upgrade_and_manual_on_off_rollback_preserve_all_five_ancestors(self):
    ...
    def test_partial_transaction_reconstruction_can_restore_64ed654f(self):
    ...
    def test_each_missing_candidate_queue_metric_restores_valid_64ed654f(self):
        for key in self.b.queue_adapter.QUEUE_METRICS:
    ...
    def test_all_operator_signals_restore_handlers_and_64ed654f(self):
...
class ModuleTests(unittest.TestCase):
    def make_dependencies(self):
        old_h = types.SimpleNamespace(RELEASE=Path('/64ed654f'), BINARY=Path('/64ed654f/gtron'))
        old_i = types.SimpleNamespace(CURRENT_SOURCE='8a0681d3', RELEASE=Path('/64ed654f/source'))
        ...
        record = {'binary_sha256': 'b' * 64, 'old_armed': marker8a}
        self.checks = []
        ...
        old_bundle = types.SimpleNamespace(original=original, i=old_i, h=old_h,
            guard=fixture.s, g=fixture.g, ancestor_markers=(markerAf989, marker8058, markerD656),
        ...
        def previous_load(args):
            ...
            return old_bundle
        previous = types.SimpleNamespace(BINARY=Path('/64ed654f/gtron'), RELEASE=Path('/64ed654f'),
        ...
        adapter = types.SimpleNamespace(configure=lambda engine: previous,
            previous_adapter=lambda: companion_adapter)
    ...
    def test_load_immutable_predecessor_does_not_compare_live_partial_transaction(self):
        ...
        self.assertEqual(bundle.ancestor_markers[1]['path'], '/af989/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[2]['path'], '/8058/reader-required.json')
        self.assertEqual(bundle.ancestor_markers[3]['path'], '/d656/reader-required.json')
        self.assertEqual(len(bundle.ancestor_markers), 4)
        ...
        for unapproved in ('/b130/gtron', '/d656/gtron', '/8058/gtron', '/af989/gtron', '/8a0681d3/gtron'):
    ...
    def test_wrong_predecessor_prepared_sha_rejected_before_loading(self):
        ...
        with patch.object(q, 'previous_adapter') as loader:
            for bad in ('', q.PREDECESSOR_PARENT_SHA, '0' * 64):
                with self.subTest(sha=bad), self.assertRaisesRegex(RuntimeError, '64ed654f predecessor'):
    ...
    def test_pinned_7af1212c_adapter_and_raw_engines_are_independent(self):
        ...
        self.assertEqual(old.CURRENT_SOURCE, '8a0681d3b62be2375dbcb0b449c4efe7b8ad5279')
        self.assertEqual(old.RELEASE, Path('/data/gtron/releases/20260914-cold-metadata'))
    ...
    def test_main_resolves_raw_engine_through_all_three_pinned_adapters(self):
        ...
        with patch.object(q, 'APPROVED', True), \
                patch.object(q, 'previous_adapter', return_value=cold_metadata) as load_adapter, \
    ...
    def test_frozen_scope_matches_commit_when_available(self):
        ...
        self.assertNotIn('scripts/tests/test_deploy_large_history_20260914.py', actual)
