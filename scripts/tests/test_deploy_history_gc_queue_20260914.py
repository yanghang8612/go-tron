"""Local-only GC reader upgrade, recovery and predecessor isolation tests."""
import copy
...
q = load('gc_queue_ops', 'scripts/deploy_history_gc_queue_20260914.py')
fixture = load('gc_queue_transaction_fixture',
               'scripts/tests/test_deploy_history_sharing_cpu_20260913.py')


def candidate_engine():
    engine = load('gc_queue_candidate_engine', q.PREVIOUS_PATH)
...
class TransactionTests(fixture.TransactionTests):
    def setUp(self):
        ...
        self.ancestor = self.root / 'd656-reader-required.json'
        self.ancestor.write_text(json.dumps({'permanent': 'D656 v3 reader'}) + '\n')
        self.ancestor_bytes = self.ancestor.read_bytes()
        self.b.ancestor_marker = fixture.item(self.ancestor, self.ancestor_bytes)
        ...
        self.metrics_include_queue = True
        self.metric_reads = 0
        ...
        p = patch.object(self.b.g, 'read_local_metrics', side_effect=self.read_metrics)
        p.start()
    ...
    def test_upgrade_and_manual_on_off_rollback_preserve_both_ancestors(self):
        self.assertTrue(fixture.g.activate_transaction(self.d)['active'])
        self.assert_permanent_markers()
        self.metrics_include_queue = False  # The actual 8058 reader has none.
    ...
    def test_missing_candidate_queue_metrics_rolls_back_to_8058(self):
        self.metrics_include_queue = False
    ...
    def test_queue_metrics_from_another_process_rejected(self):
        ...
        def changed():
            ...
            if self.metric_reads == 2:
                value['process/start/unix_nano']['value'] += 1
            return value
    ...
    def test_ancestor_marker_change_rejected_before_stop(self):
        self.ancestor.write_bytes(b'altered ancestor')
        with self.assertRaisesRegex(RuntimeError, 'ancestor permanent'):
            self.d.stop()
        self.assertNotIn(('stop',), self.calls)

    def test_all_operator_signals_restore_handlers_and_8058(self):
        for sig in (signal.SIGINT, signal.SIGTERM, signal.SIGHUP):
            with self.subTest(signal=sig):
                self.failure = str(self.data_marker)
                self.failed = False
...
class ModuleTests(unittest.TestCase):
    def make_dependencies(self):
        old_h = types.SimpleNamespace(RELEASE=Path('/8058'), BINARY=Path('/8058/gtron'))
        old_i = types.SimpleNamespace(CURRENT_SOURCE='d656', RELEASE=Path('/8058/source'))
        marker = {'path': '/d656/reader-required.json', 'sha256': 'd' * 64}
        record = {'binary_sha256': 'b' * 64, 'old_armed': marker}
        self.checks = []
        ...
        old_bundle = types.SimpleNamespace(original=original, i=old_i, h=old_h,
                                           guard=fixture.s, g=fixture.g)
        self.previous_args = []
        def previous_load(args):
            self.previous_args.append(copy.deepcopy(vars(args)))
            return old_bundle
        previous = types.SimpleNamespace(BINARY=Path('/8058/gtron'), RELEASE=Path('/8058'),
        ...
        return previous, builder, wrapper

    def test_loading_prepared_uses_immutable_evidence_without_live_comparison(self):
        engine = candidate_engine()
        previous, builder, wrapper = self.make_dependencies()
        with patch.object(q, 'APPROVED', True), patch.object(q, 'ALLOWED_FILES', ('candidate.go',)), \
                patch.object(q, 'previous_module', return_value=previous) as loader, \
        ...
        loader.assert_called_once_with(q.PREVIOUS_NATIVE_SCRIPT)
        self.assertEqual(self.checks, [])
        self.assertEqual(self.previous_args, [{'mode': 'activate', 'source_revision': q.CURRENT_SOURCE,
            'script_revision': q.PREVIOUS_REVISION, 'old_prepared_sha': q.ANCESTOR_PREPARED_SHA,
        ...
        self.assertEqual(builder.CURRENT_PID, q.CURRENT_PID)
        self.assertEqual(builder.CURRENT_TICKS, q.CURRENT_TICKS)
        self.assertEqual(builder.SCRIPT_PATH, q.SCRIPT_PATH)
        self.assertEqual(builder.ALLOWED_FILES, ('candidate.go',))
        self.assertIs(bundle.ancestor_marker, self.old_record['old_armed'])
        ...
        with self.assertRaises(RuntimeError):
            bundle.h.expected_history_queue_environment('/b130/gtron')
    ...
    def test_wrong_predecessor_prepared_sha_rejected_before_loading(self):
        engine = candidate_engine()
        with patch.object(q, 'previous_module') as loader:
            for bad in ('', q.ANCESTOR_PREPARED_SHA, '0' * 64):
                with self.subTest(sha=bad), self.assertRaisesRegex(RuntimeError, '8058 predecessor'):
    ...
    def test_pending_review_refuses_before_dependency_execution(self):
        ...
        with patch.object(q, 'APPROVED', False), patch.object(q, 'previous_module') as loader:
            with self.assertRaisesRegex(RuntimeError, 'not frozen'):
                q.main()
            loader.assert_not_called()

    def test_engine_blob_sha_checked_before_execution(self):
        with patch.object(q.subprocess, 'check_output', return_value=b'raise AssertionError("executed")'):
            with self.assertRaisesRegex(RuntimeError, 'engine blob changed'):
                q.previous_module('/tmp/native.py')

    def test_pinned_engine_source_is_exact(self):
        with patch.object(q, 'REPO', ROOT):
            previous = q.previous_module(q.PREVIOUS_NATIVE_SCRIPT)
        self.assertEqual(previous.__file__, q.PREVIOUS_NATIVE_SCRIPT)
        self.assertEqual(previous.SOURCE_REVISION, q.CURRENT_SOURCE)
        self.assertEqual(previous.CURRENT_SOURCE, 'd656be3c43eb237fac4d46a8847e5fe551bc6a78')
        self.assertEqual(previous.RELEASE, Path('/data/gtron/releases/20260913-history-sharing-cpu'))
    ...
    def test_queue_metric_contract_rejects_missing_bool_float_and_negative(self):
        valid = {'state/history/shared/gc/' + key: {'value': 0} for key in q.QUEUE_METRICS}
        q.validate_queue_metrics(valid)
        for key in q.QUEUE_METRICS:
            for bad in (None, True, 1.0, -1, '1'):
                with self.subTest(key=key, bad=bad):
                    metrics = copy.deepcopy(valid)
                    if bad is None:
                        del metrics['state/history/shared/gc/' + key]
                    else:
                        metrics['state/history/shared/gc/' + key]['value'] = bad
                    with self.assertRaisesRegex(RuntimeError, 'GC metric'):
                        q.validate_queue_metrics(metrics)
