"""Local-only companion upgrade: recovery, ancestor retention and attestation."""
import copy
...
q = load('companion_lookup_ops', 'scripts/deploy_history_companion_lookup_20260914.py')
fixture = load('companion_transaction_fixture',
...
def candidate_engine():
    ...
    engine = adapter.previous_module('/tmp/test-history-companion-engine.py')
...
class TransactionTests(fixture.TransactionTests):
    def setUp(self):
        ...
        self.b.queue_adapter = predecessor_adapter()
    ...
    def test_every_file_replace_and_reload_failure_restores_af989_on(self):
    ...
    def test_upgrade_and_manual_on_off_rollback_preserve_all_three_ancestors(self):
    ...
    def test_partial_transaction_reconstruction_can_restore_af989(self):
    ...
    def test_each_missing_candidate_queue_metric_restores_valid_af989(self):
        for key in self.b.queue_adapter.QUEUE_METRICS:
    ...
    def test_all_operator_signals_restore_handlers_and_af989(self):
...
class ModuleTests(unittest.TestCase):
    def make_dependencies(self):
        old_h = types.SimpleNamespace(RELEASE=Path('/af989'), BINARY=Path('/af989/gtron'))
        old_i = types.SimpleNamespace(CURRENT_SOURCE='8058', RELEASE=Path('/af989/source'))
        marker8058 = {'path': '/8058/reader-required.json', 'sha256': '8' * 64}
        markerD656 = {'path': '/d656/reader-required.json', 'sha256': 'd' * 64}
        record = {'binary_sha256': 'b' * 64, 'old_armed': marker8058}
        self.checks = []
        ...
        old_bundle = types.SimpleNamespace(original=original, i=old_i, h=old_h,
            guard=fixture.s, g=fixture.g, ancestor_marker=markerD656)
        self.previous_args = []
        def previous_load(args):
            self.previous_args.append(copy.deepcopy(vars(args)))
            return old_bundle
        previous = types.SimpleNamespace(BINARY=Path('/af989/gtron'), RELEASE=Path('/af989'),
            load=previous_load, Deployment=lambda *args: predecessor)
        self.previous_paths = []
        adapter = types.SimpleNamespace(configure=lambda engine: previous,
            previous_module=lambda path: self.previous_paths.append(path))
    ...
    def test_load_immutable_predecessor_does_not_compare_live_partial_transaction(self):
        ...
        self.assertEqual(builder.NATIVE_TEST_PATTERN,
            'Test.*(History|Shared|StateChange|StateDomainChangePrune|AsOf|Unwind|ResetMutableState|Snapshot.*Coverage|Manifest|Companion)')
        self.assertIs(bundle.ancestor_markers[0], self.old_record['old_armed'])
        self.assertEqual(bundle.ancestor_markers[1]['path'], '/d656/reader-required.json')
        ...
        for unapproved in ('/b130/gtron', '/d656/gtron', '/8058/gtron'):
    ...
    def test_wrong_predecessor_prepared_sha_rejected_before_loading(self):
        ...
        with patch.object(q, 'previous_adapter') as loader:
            for bad in ('', q.PREDECESSOR_PARENT_SHA, '0' * 64):
                with self.subTest(sha=bad), self.assertRaisesRegex(RuntimeError, 'af989 predecessor'):
    ...
    def test_pinned_b182_adapter_and_raw_engines_are_independent(self):
        old_adapter = predecessor_adapter()
        candidate = candidate_engine()
        old_raw = old_adapter.previous_module(q.PREVIOUS_NATIVE_SCRIPT)
        ...
        self.assertEqual(old.CURRENT_SOURCE, '8058e99d4e080c3329a587a2feef16133e676e84')
        self.assertEqual(old.RELEASE, Path('/data/gtron/releases/20260914-history-gc-queue'))
    ...
    def test_frozen_scope_matches_commit_when_available(self):
        ...
        self.assertNotIn('scripts/tests/test_deploy_history_companion_lookup_20260914.py', actual)
