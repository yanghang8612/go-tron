...
PATH = ROOT / 'scripts/verify_history_reader_20260914.py'
spec = importlib.util.spec_from_file_location('reader_acceptance', str(PATH))
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)


def fixture_rows():
    # Deliberately independent enumeration, shuffled metric order and custom
    # units; this should catch a missing fixture or positional parser.
    lines = ['goos: linux', 'goarch: amd64']
    for fixture in ('tiny_raw', 'small_snappy', 'raw_unique', 'raw_repeated',
                    'snappy_unique', 'snappy_repeated', 'mixed_unique'):
        for reader in ('has_get', 'coupled'):
            for mode in ('legacy', 'candidate'):
                for i in range(5):
                    lines.append('BenchmarkSharedHistoryReaderFullPack/{}/{}/{}-2 100 '
                                 '{} allocs/op 7 MB/s {} B/op {:.2e} ns/op 123 raw_B/op'.format(
                                     fixture, reader, mode, i, i * 10, (i + 1) * 100.0))
...
class ParserTests(unittest.TestCase):
    def test_28_cases_140_rows_and_named_units(self):
        result = runner.parse_benchmarks(fixture_rows())
        ...
        self.assertEqual(case['metrics']['raw_B/op']['median'], 123.0)
    ...
    def test_missing_extra_and_malformed_rows_never_pass(self):
        ...
        invalid = [
            ...
            original.replace('/tiny_raw/', '/unknown/', 1),
            ...
            original.replace(' 7 MB/s', ' 7 B/op', 1),
            original.replace('123 raw_B/op', '123 other/op', 1),
            original.replace('123 raw_B/op', '123', 1),
        ]
        for index, text in enumerate(invalid):
            with self.subTest(index=index), self.assertRaises((RuntimeError, ValueError)):
                runner.parse_benchmarks(text)
...
class NativeBoundaryTests(unittest.TestCase):
    ...
    def test_exact_source_order_env_and_complete_command_artifacts(self):
        ...
        self.assertEqual(record['reader_benchmark']['sample_count'], 140)
        ...
        self.assertEqual(self.calls[1][0], ['/data/go/bin/go', 'test', '-p', '2', '-tags', 'sapling',
            './core/state', './core/state/snapshots', './core/state/pruning', './core/freezer', './actuator', '-count=1', '-timeout=300s'])
        self.assertEqual(self.calls[2][0], ['/data/go/bin/go', 'test', '-p', '2', '-tags', 'sapling', './core/rawdb',
            '-run', '^$', '-bench', '^BenchmarkSharedHistoryReaderFullPack$', '-benchmem', '-benchtime=200ms', '-count=5', '-timeout=180s'])
    ...
    def test_failed_tests_stop_and_incomplete_benchmark_is_not_complete(self):
        ...
        self.assertFalse(self.summary()['complete'])
        self.fail_tests = False
        self.bench = '\n'.join(self.bench.splitlines()[1:3] + self.bench.splitlines()[4:])
