...
RELEASE = Path('/data/gtron/releases/20260913-range-scheduling')
...
BASE_PREFIX = '4940853af2424d04b86236e4ec682dcfc32e75e2'
...
RELEASE_KIND = 'range-scheduling-git-20260913'
SCRIPT_PATH = 'scripts/deploy_range_scheduling_20260913.py'
...
HISTORY_QUEUE_ENV = 'GTRON_HISTORY_RANGE_QUEUE'
# Frozen after the source tests and independent safety review passed.
QUEUE_SCHEDULING_APPROVED = True
QUEUE_METRIC_CONTRACT_FROZEN = True
...
SOURCE_REVISION = 'fdfe853236445ca77c2d7a9553c1d4e172ff9a43'
EXPECTED_FILES = {
    "cmd/gtron/history_pruner_adapter.go": "5969939a06945f0acb55646cfaa8475198af2dba591a0f416929a742418a79c9",
    "cmd/gtron/history_pruner_guard_adapter_test.go": "1fb448f2898e0d60ded88fe1dfc4d96f00637fabc18f2bbdedc85c9ccb515292",
    "cmd/gtron/history_range_prune.go": "bbb7b44f75d8526a90c730fd2e18f1e742a34650717b55b468ad89d8d61f422d",
    "cmd/gtron/history_range_prune_test.go": "1a1857499d4390a02bcc479810d4ceed2bb1070c914a75476e846715c34c7600",
    "cmd/gtron/main.go": "06004b3b7cf5efd74aa6760575454ef96561e5ef5c4adcc5597f8c4585cc850a",
    "core/state/pruning/history_range_queue_pruner_test.go": "5209d539a88eef4d1b3bbc81c35e22c9932a50179b07d2857814f4b6e3cd5947",
    "core/state/pruning/history_range_queue_worker_test.go": "15103ac964358b104391c9e72b83c9e321eb35b7bf1dcf8579c4b88418432784",
    "core/state/pruning/pruner.go": "b81f8f41a9a2ad3b4109d3039884300c0b94f83f6cd5bbca4f0ecb71f069397e",
    "core/state/pruning/worker.go": "8b92b7aa3e339f08514b9d211ec7fdd26bb8407c2928bcd83a42a9bff98a84ff",
    "core/state_domain_change_prune_guard.go": "435836995c1643a3707dc5c710726e67b1e07e0fc113742c6be574c3bcadb30d",
    "core/state_domain_change_prune_guard_metrics.go": "f68cfc2958b957939ae10499606eaa44a1df663b6fbf250bee8da6d6362a6a83",
    "core/state_domain_change_prune_guard_test.go": "ceced61575a3f2e159df1f48b9f33929c73ad3bd9836ba4254f433d4f5a0ba6b",
    "core/state_domain_change_prune_queue_test.go": "b127baca8da39b12b86cb9ca1d4f558093825520f10c7c5a94a2d433ae1d0d6b",
    "scripts/deploy_reclaim_optimization_20260912.py": "12db7f2f9fa4fa3e61e5c76e90a4529dd8f85e60e1fd7402219f8e338cbe3c2d"
}
# Already deployed contract: required for both candidate and old/rollback health.
...
# Candidate-only contract; zero activity at startup is valid. Old/rollback
# health requires the existing 31 fields, never these 15 new fields.
QUEUE_METRIC_CHECKS = (
    {'name': 'state/prune/history/delete/range/queue/enabled', 'field': 'value', 'operator': 'eq', 'expected': 1},
) + tuple(
    {'name': 'state/prune/history/delete/range/guard/' + name, 'field': 'count', 'operator': 'ge', 'expected': 0}
    for name in ('attempts', 'busy_index', 'busy_chain', 'admitted',
                 'prefix_unsettled', 'proof_errors', 'other_errors', 'work_errors')
) + tuple(
    {'name': 'state/prune/history/delete/range/guard/queued/' + name,
     'field': 'count', 'operator': 'ge', 'expected': 0}
    for name in ('attempts', 'chain_busy', 'chain_wait/total_ns', 'chain_held/total_ns')
) + tuple(
    {'name': 'state/prune/history/delete/range/guard/queued/' + name,
     'field': 'value', 'operator': 'ge', 'expected': 0}
    for name in ('chain_wait/max_ns', 'chain_held/max_ns')
)
...
def history_queue_process_environment(pid):
    return process_environment_value(pid, HISTORY_QUEUE_ENV)


def expected_history_queue_environment(executable):
    require(executable in (OLD_EXE, str(BINARY)), 'unrecognized release for history-queue environment')
...
def verify_observer_runtime(pid, expected_exe):
    ...
    expected_queue = expected_history_queue_environment(expected_exe)
    require(history_queue_process_environment(pid) == expected_queue,
            'history-queue process environment differs for expected release')
    ...
    verify_metric_checks(metrics, RECLAIM_METRIC_CHECKS, 'existing reclaim')
    reclaim = {check['name']: metrics[check['name']][check['field']]
               for check in RECLAIM_METRIC_CHECKS}
    queue = {}
    if expected_queue == '1':
        require(QUEUE_METRIC_CONTRACT_FROZEN is True, 'queue metric contract is not frozen')
        verify_metric_checks(metrics, QUEUE_METRIC_CHECKS, 'range queue candidate')
        queue = {check['name']: metrics[check['name']][check['field']]
                 for check in QUEUE_METRIC_CHECKS}
    return {'process_start_unix_nano': process_start,
            'cache_observer': values,
            'history_range_environment': expected_range,
            'existing_reclaim': reclaim,
            'history_queue_environment': expected_queue,
            'range_queue_candidate': queue,
...
def process_identity(pid):
    ...
    return {'pid': pid, 'exe': exe, 'exe_sha256': file_sha(root / 'exe'),
            ...
            'history_queue_environment': history_queue_process_environment(pid),
...
def preflight(expected_pid=None, expected_exe=None, expected_sha=None):
    ...
    expected_queue = expected_history_queue_environment(process['exe'])
    require(process['history_queue_environment'] == expected_queue,
            'history-queue process environment changed from expected release')
    require(environment_settings(main['properties']).get(HISTORY_QUEUE_ENV) == expected_queue,
            'history-queue unit environment changed from expected release')
    if expected_exe:
...
def effective_exec_file(snapshot):
    ...
    for item in snapshot['main']['files']:
        ...
        require(HISTORY_QUEUE_ENV not in text, 'history-queue already mentioned in original unit files')
        section = ''
        logical = ''
...
def verify_after_configuration(before, now, item, updated):
    ...
    require(HISTORY_QUEUE_ENV not in expected_environment, 'history-queue already configured before activation')
    expected_environment[HISTORY_QUEUE_ENV] = '1'
    require(environment_settings(now['main']['properties']) == expected_environment,
            'effective environment differs beyond approved history-queue addition')
    ...
    require(now['process']['history_queue_environment'] == '1',
            'running history-queue environment differs')
...
def validate_revision(revision, script_revision):
    ...
    validate_metric_contract(RECLAIM_METRIC_CHECKS, 'existing reclaim')
    require(QUEUE_METRIC_CONTRACT_FROZEN is True, 'queue metric contract is not frozen')
    validate_metric_contract(QUEUE_METRIC_CHECKS, 'range queue candidate')
    require(QUEUE_SCHEDULING_APPROVED is True, 'queue scheduling rollout decision is not frozen')
...
def prepare(args):
    ...
    env.pop(HISTORY_QUEUE_ENV, None)
    ...
    progress('prepare: native storage and maintenance tests with all five opt-ins enabled')
    observer_env = dict(env)
    observer_env[OBSERVER_ENV] = '1'
    observer_env[PRUNE_ENV] = '1'
    observer_env[HISTORY_RANGE_ENV] = '1'
    observer_env[HISTORY_QUEUE_ENV] = '1'
    ...
    test_pattern = ('Test.*(StateChangePostingPruneGuard|PostingPrune|StateDomainChangePruneGuard|StateDomainChangePruneQueue|DomainPruner.*GuardAdapter|HistoryRangeQueue|HistoryRangePrune|HotHistoryPrune|'
...
def wait_healthy(expected_exe, expected_sha, expected_argv, seconds=240):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            require(proc['history_queue_environment'] == expected_history_queue_environment(expected_exe),
                    'history-queue environment differs for expected executable')
            head = wallet_head()
...
def activation_files(record):
    ...
    require(HISTORY_QUEUE_ENV.encode() not in original, 'history-queue already mentioned in original ExecStart file')
    ...
    addition = ('\n[Service]\nEnvironment=' + HISTORY_QUEUE_ENV + '=1\n').encode()
