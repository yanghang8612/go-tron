...
RELEASE = Path('/data/gtron/releases/20260912-posting-prune')
SOURCE = RELEASE / 'source'
BINARY = RELEASE / 'gtron'
OLD_EXE = '/data/gtron/releases/20260912-db-space-observation/gtron'
OLD_SHA = '4e2eb51c88ae262f2e3e83a4eb6f5598146864fd11fa1f9e57c9ec9e6c79998e'
OLD_PID = 7759
OLD_START_TICKS = 4491079649
BASE_PREFIX = '5664a53e87b20e05ff1204f40b026895d6f1f9db'
...
RELEASE_KIND = 'posting-prune-git-20260912'
SCRIPT_PATH = 'scripts/deploy_posting_prune_20260912.py'
...
SOURCE_REVISION = '7cf097c9d004d417b7157216420ec79b123ffc2b'
EXPECTED_FILES = {
    "cmd/gtron/main.go": "441b02252b7879d2d07cd48b5f3f8b07c92282525f9e1ea417ff06cbce272767",
    "cmd/gtron/posting_prune.go": "60649d8e6caf7b9c01172e289e6f3c805302b001eb74749d25893e6e468374cd",
    "cmd/gtron/posting_prune_test.go": "c4dc9a0eb11a468e4564a717248f6d8dcbc8b8114da15c098e568ee490227438",
    "core/rawdb/accessors_state_change_posting_chunk.go": "9ef5c20c5898bf64c1ebd7be38feb411946b221c679037c220fc07edfb636190",
    "core/rawdb/accessors_state_change_posting_chunk_test.go": "012b3c165b5b46a4cc3ef206fcf0ef95f906ebeb18a5ee85ccfc930ee2a316c3",
    "core/state/pruning/lifecycle.go": "739209ef8ca2c2b4928aed6a3523527b201c2378b065f032314c249c75526517",
    "core/state/pruning/posting_permission_test.go": "3d6cdc82edbbc76170b79507cf2b48476f633ef21bebe409fa5954d6b5970709",
    "core/state/pruning/posting_worker.go": "24f6f04b11b741f895bcede49700ab847802bc0bf7ca93b602ee775edde46198",
    "core/state/pruning/posting_worker_test.go": "4cf923f098b69fae84f9e59f30eb96e6708686ef4246ea93ab4abb7196c160c5",
    "core/state/pruning/pruner.go": "6542044471a884a7be14de7d6e9708343f7595bb92939bd048a2e8baea76663f",
    "core/state_change_posting_prune.go": "9cdf7b63e0788d4a7e0500dad98ed66fe95488a22733c2cbfd861bd13cdbcfa3",
    "core/state_change_posting_prune_test.go": "66db55e700aca600a168251693af87b01f943ef2ba23f0fa743644f3862f3178",
    "scripts/deploy_db_space_observation_20260912.py": "854517313dd145d500dac8af0249162109334dcda26444f8a7d438fc057e36ba"
}
...
def verify_observer_runtime(pid, include_prune=True):
    ...
    require(posting_prune_process_environment(pid) == ('1' if include_prune else None),
            'posting-prune process environment differs from expected release')
    ...
    return {'process_start_unix_nano': process_start,
            ...
            'posting_prune': {check['name']: metrics[check['name']][check['field']]
                              for check in PRUNE_METRIC_CHECKS} if include_prune else None}
...
def preflight(expected_pid=None, expected_exe=None, expected_sha=None):
    ...
    expected_prune = '1' if expected_exe == str(BINARY) else None
    require(process['posting_prune_environment'] == expected_prune,
            'posting-prune process environment changed from expected release')
    require(environment_settings(main['properties']).get(PRUNE_ENV) == expected_prune,
            'posting-prune unit environment changed from expected release')
    if expected_prune is None:
        require(all(PRUNE_ENV.encode() not in base64.b64decode(item['data_b64'])
                    for item in main['files']),
                'posting-prune text already present in old unit files; inspect before preparing')
...
def verify_after_configuration(before, now, item, updated):
    ...
    require(PRUNE_ENV not in expected_environment, 'posting-prune assignment already present in old unit')
    expected_environment[PRUNE_ENV] = '1'
    require(environment_settings(now['main']['properties']) == expected_environment,
            'effective environment changed beyond posting-prune assignment')
...
def wait_healthy(expected_exe, expected_sha, expected_argv, seconds=240):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            require(proc['posting_prune_environment'] == ('1' if expected_exe == str(BINARY) else None),
                    'posting-prune environment differs from expected release')
            head = wallet_head()
            ...
            if first is None or identity != first_identity:
                ...
                observer = verify_observer_runtime(proc['pid'], include_prune=expected_exe == str(BINARY))
...
def activation_files(record):
    ...
    require(updated.replace(str(BINARY).encode(), OLD_EXE.encode(), 1) == original,
            'candidate config differs beyond executable before posting-prune assignment')
    require(PRUNE_ENV.encode() not in original, 'posting-prune text already present in active unit file')
    updated = updated.rstrip(b'\n') + b'\n\n[Service]\nEnvironment=' + PRUNE_ENV.encode() + b'=1\n'
