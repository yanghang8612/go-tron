#!/usr/bin/env python3
"""Native commitment-prefetch acceptance with pinned helpers; Python 3.6 compatible."""
import argparse
import collections
import contextlib
import datetime
...
OUTPUT = Path('/tmp/gtron-commitment-prefetch-acceptance-20260914')
...
BENCHMARK_ORDER = ('prefetch', 'event_frontier')
BENCHMARKS = {
    'prefetch': {
        'name': 'BenchmarkCommitmentPrefetchOwnershipFullSession',
        'package': './core/blockbuffer',
        'paths': tuple('{}/{}/{}'.format(key, workload, mode)
    ...
    'event_frontier': {
        'name': 'BenchmarkEventFrontierAllocation',
        'package': './core/state/snapshots',
        'paths': tuple('{}/{}/{}'.format(operation, catalog, variant)
...
PACKAGES = ('./core/blockbuffer', './core/state/domains', './core/state',
            './core/state/snapshots', './core/state/pruning')
REPEATS = 5
...
def load_helper():
    ...
    helper = types.ModuleType('commitment_prefetch_native_helper')
...
def expected_cases(kind):
    require(kind in BENCHMARKS, 'unknown benchmark kind')
    spec = BENCHMARKS[kind]
    paths = spec['paths']
    require(spec['name'] and spec['package'] and paths and len(set(paths)) == len(paths),
            kind + ' benchmark review not frozen')
    return set(spec['name'] + '/' + path for path in paths)


def parse_benchmarks(text, kind):
    expected = expected_cases(kind)
    benchmark = BENCHMARKS[kind]['name']
    ...
    require(len(cases) == len(expected) and count == len(expected) * REPEATS,
            kind + ' benchmark case/sample count differs')
    ...
    with open(str(OUTPUT / 'runner.lock'), 'a') as lock:
        ...
        state = {'complete': False, 'started_utc': h.utc_now(), 'invocation_argv': list(sys.argv),
                 ...
                 'limitations': ['All commands use source fixtures; no production datadir or service operation is issued.',
                                 ...
                                 'Complete means commands passed and all approved cases have five valid rows, not a speedup gate or deployment approval.',
        ...
        with interrupted_as_error():
            try:
                ...
                for kind in BENCHMARK_ORDER:
                    spec = BENCHMARKS[kind]
                    text = h.run_command(kind + '-benchmark', common + [spec['package'], '-run', '^$', '-bench',
                                         '^' + spec['name'] + '$', '-benchmem', '-benchtime=200ms', '-count=5', '-timeout=180s'],
                                         env, directory, 240, state)
                    state['benchmarks'][kind] = parse_benchmarks(text, kind)
                    h.write_json(summary_path, state)
                ...
                require([record['name'] for record in state['commands']] ==
                        ['go-version', 'related-package-tests'] + [kind + '-benchmark' for kind in BENCHMARK_ORDER],
                        'command evidence incomplete')
                ...
                print(h.json.dumps({'complete': state['complete'], 'summary': str(summary_path),
                                    ...
                                    'benchmarks': {kind: {'cases': report['case_count'], 'samples': report['sample_count']}
                                                   for kind, report in state.get('benchmarks', {}).items()}},
