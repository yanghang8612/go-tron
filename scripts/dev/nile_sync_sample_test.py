...
class NileSampleHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if method == "gtron_freezerStatus":
            ...
            result = {
                "number": "0x63",
                "hash": request.get("params", [block_hash])[0],
                "transactions": [tx_hash],
            }
            ...
            if getattr(self.server, "invalid_trace_block", False):
                result = "0x0"
            else:
                result = [
                    {
...
class NileSyncSampleTest(unittest.TestCase):
    ...
    def test_archive_api_probe_rejects_invalid_trace_block_result(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            server = ThreadingHTTPServer(("127.0.0.1", 0), NileSampleHandler)
            server.invalid_trace_block = True
            thread = threading.Thread(target=server.serve_forever, daemon=True)
            thread.start()
            self.addCleanup(server.shutdown)
            self.addCleanup(server.server_close)
    ...
    def test_sample_includes_snapshot_manifest_profile_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prom_text = prometheus.read_text(encoding="utf-8")
            self.assertIn("gtron_nile_sync_snapshot_profile_verify_files", prom_text)
            self.assertIn("gtron_nile_sync_snapshot_profile_verified_segments", prom_text)
            ...
            self.assertIn(
                f"gtron_nile_sync_snapshot_point_tx_hash_lookup_snapshot_share_milli{{{labels}}} 60",
                metrics,
            )
            self.assertIn(
                f"gtron_nile_sync_snapshot_state_history_compressed_segments{{{labels}}} 1",
                metrics,
            )
            self.assertIn(
                f"gtron_nile_sync_snapshot_state_history_compressed_bytes{{{labels}}} 80",
                metrics,
            )
            self.assertIn(
                f"gtron_nile_sync_snapshot_state_history_compressed_share_milli{{{labels}}} 1000",
                metrics,
            )
            ...
            self.assertIn(
                f"gtron_nile_sync_snapshot_point_event_log_index_snapshot_share_milli{{{labels}}} 120",
                metrics,
            )
    ...
    def test_sample_writes_prometheus_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(f'gtron_nile_sync_signed_cold_prune{{{labels}}} 0', metrics)
            self.assertIn(f'gtron_nile_sync_cold_freezer_to_block{{{labels}}} -1', metrics)
            self.assertIn(f'gtron_nile_sync_chain_lookup_prune_to_block{{{labels}}} -1', metrics)
            self.assertIn(f'gtron_nile_sync_tail_pruned_through_block{{{labels}}} -1', metrics)
            ...
            estimate_gas_labels = f'datadir="{datadir}",label="candidate",method="eth_estimateGas",mode="full",network="nile"'
            self.assertIn(f'gtron_nile_sync_archive_api_method_success{{{estimate_gas_labels}}} 1', metrics)
            ...
            block_trace_labels = f'datadir="{datadir}",label="candidate",method="debug_traceBlockByNumber",mode="full",network="nile"'
            self.assertIn(f'gtron_nile_sync_archive_api_method_success{{{block_trace_labels}}} 1', metrics)
    ...
    def test_sample_includes_sync_health_and_disk_ratios(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(row["syncLogStatePrefetchEnqueued"], 31)
            self.assertEqual(row["syncLogStatePrefetchDropped"], 2)
            self.assertEqual(row["syncLogStatePrefetchProcessed"], 29)
    ...
    def test_sample_parses_json_sync_log_segment(self):
        with tempfile.TemporaryDirectory() as tmp:
                ...
                json.dumps(
                    {
                        ...
                        "statePrefetchDropped": 1,
                        "statePrefetchProcessed": 11,
                        "statePrefetchHits": 8,
            ...
            self.assertEqual(row["syncLogSegmentExecElapsed"], "1500ms")
            self.assertEqual(row["syncLogSegmentApplyElapsed"], "1700ms")
            self.assertEqual(row["syncLogStatePrefetchEnqueued"], 12)
            self.assertEqual(row["syncLogStatePrefetchDropped"], 1)
            self.assertEqual(row["syncLogStatePrefetchProcessed"], 11)
            self.assertEqual(row["syncLogStatePrefetchHits"], 8)
            self.assertEqual(row["syncLogStatePrefetchMisses"], 3)
            self.assertEqual(row["syncLogStatePrefetchErrors"], 0)
    ...
    def test_sample_preserves_offline_storage_alert_details(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_gtron.write_text(
                "\n".join(
                    [
                        ...
                        'gtron_storage_prune_boundary_block{datadir="/tmp/nile",field="derivedIndexToBlock"} 11',
            ...
            self.assertEqual(row["derivedIndexToBlock"], 11)
