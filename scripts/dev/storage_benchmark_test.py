...
class StorageBenchmarkTest(unittest.TestCase):
    ...
    def test_archive_api_probe_rejects_expected_state_mismatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(row["archiveApiExpectedCode"], "0x0")
            self.assertEqual(row["archiveApiExpectedStorage"], "0x1")
            ...
            self.assertIn("eth_getBalance", row["archiveApiMethods"])
            self.assertIn("eth_getCode", row["archiveApiMethods"])
            self.assertNotIn("eth_getStorageAt", row["archiveApiMethods"])
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_gtron.write_text(
                textwrap.dedent(
                    ...
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="coldFreezerToBlock"} 12
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="derivedIndexToBlock"} 11
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="chainLookupPruneToBlock"} 10
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="tailPrunedThroughBlock"} 8
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="balanceTracePruneToBlock"} 7
                    gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="sectionBloomPruneToSection"} 6
                    # TYPE gtron_storage_alert_issue gauge
            ...
            self.assertEqual(row["signedColdPrune"], 1)
            ...
            self.assertIn('gtron_storage_signed_cold_prune{datadir="/tmp/gtron"} 1', metrics)
            self.assertIn(
                'gtron_storage_prune_boundary_block{datadir="/tmp/gtron",field="derivedIndexToBlock"} 11',
                metrics,
            )
