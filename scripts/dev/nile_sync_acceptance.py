...
def check_archive_api_evidence(
    ...
    if field_present(row, "coldFreezerToBlock"):
        ...
        if cold_freezer is None:
            issues.append(
                f"coldFreezerToBlock={row.get('coldFreezerToBlock')!r}, want integer"
            )
    chain_lookup = None
    ...
    if require_post_prune:
        ...
        if not has_prune_boundary:
            ...
            issues.append(
                f"coldFreezerToBlock={row.get('coldFreezerToBlock')!r} must cover "
                f"archiveApiBlock={block:g} for post-prune archive API evidence"
            )
...
def check_event_log_index_evidence(row, require_non_empty=False):
    ...
    if require_non_empty:
        ...
        if topic_postings is None or topic_postings <= 0:
            issues.append(
                f"eventLogIndexTopicPostings={row.get('eventLogIndexTopicPostings')!r}, want > 0"
            )
    return issues
