...
def check_archive_api_evidence(
    ...
    for row in evidence_rows:
        ...
        if methods is None:
            ...
            if malformed:
                issues.append(
                    f"{line_label(row)} archiveApiMethods contains non-string or empty entries: "
                    + ",".join(malformed)
                )
            duplicates = duplicate_string_list_entries(row, "archiveApiMethods")
            if duplicates:
                issues.append(
                    f"{line_label(row)} archiveApiMethods contains duplicate methods: "
                    + ",".join(duplicates)
                )
...
def check_archive_tx_evidence(rows, required_modes=(), require_trace_transaction=False):
    ...
    for row in evidence_rows:
        ...
        if tx_methods is None:
            ...
            if malformed:
                issues.append(
                    f"{line_label(row)} archiveApiTxMethods contains non-string or empty entries: "
                    + ",".join(malformed)
                )
            duplicates = duplicate_string_list_entries(row, "archiveApiTxMethods")
            if duplicates:
                issues.append(
                    f"{line_label(row)} archiveApiTxMethods contains duplicate methods: "
                    + ",".join(duplicates)
                )
