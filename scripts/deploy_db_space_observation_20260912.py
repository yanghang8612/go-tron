...
RELEASE = Path('/data/gtron/releases/20260912-db-space-observation')
SOURCE = RELEASE / 'source'
BINARY = RELEASE / 'gtron'
OLD_EXE = '/data/gtron/releases/20260911-commitment-cache-observation/gtron'
OLD_SHA = 'b09df4ade279779a0933ac08b8637e3496c8b97afabf76d72cece93f4a5c3cca'
OLD_PID = 9164
BASE_PREFIX = '2fa0f2049a3996a3c43b58d436718fbdc65c2983'
...
RELEASE_KIND = 'db-space-observation-git-20260912'
SCRIPT_PATH = 'scripts/deploy_db_space_observation_20260912.py'
...
SOURCE_REVISION = '5664a53e87b20e05ff1204f40b026895d6f1f9db'
EXPECTED_FILES = {
    "core/rawdb/database.go": "06931f2c190e206d87868247ab1d641a4169825dad7b268fb792a7f771de28d5",
    "core/rawdb/pebbledb/engine_space_metrics.go": "eb086159b45dce0a515f5e0b960755aa8ce0c460c87c85fd05b61c47c41f6239",
    "core/rawdb/pebbledb/engine_space_metrics_test.go": "097f46921f41baf5620696c523185fa56d1239e64b52ff18467b60f0b7654a00",
    "core/rawdb/pebbledb/pebble.go": "370653c9620bbe9fc8a68c3597d6e4fe29694639061b9073f820e4515f7e5427",
    "core/rawdb/pebbledb/space_observer.go": "0e3f270a17299403be028e070173ccb33a22ba04e485d2ac7872c8058030eb1e",
    "core/rawdb/pebbledb/space_observer_test.go": "2e3cf6552737aed5dd12c6264143749a5d6a5a05a5f3e56dba50d5a7b1ddfe52",
    "core/rawdb/schema.go": "97932fca687a9587f428fc5214580172d9fa63c4feb9800f7af3890a54449a73",
    "core/rawdb/space_observation.go": "237eec3aecd00e852b24d81b0cbab5264e99489fe68e9b6410e7efb13313faf0",
    "core/rawdb/space_observation_test.go": "b91158f8fe8065851d14c037dfc42c8ea8486b165b98bfd1bf9a57e5060af603",
    "scripts/deploy_commitment_cache_observation_20260911.py": "3a78120cf1ec2cd1750b7447caf90e6858e16455fcf031ef3c7d11cb31336833"
}
...
DB_SPACE_METRIC_CHECKS = tuple(
    ...
    for name in ('sst/obsolete/bytes', 'sst/zombie/bytes', 'wal/obsolete/physical_bytes',
...
def verify_db_space_metrics(metrics):
    ...
    for check in DB_SPACE_METRIC_CHECKS:
        ...
        require(type(value) is int, 'missing/noninteger DB-space metric: ' + check['name'])
        expected = check['expected']
        ok = (value == expected if check['operator'] == 'eq' else
              value >= expected if check['operator'] == 'ge' else value > expected)
        require(ok, 'DB-space metric readiness failed: ' + check['name'])
...
def verify_after_configuration(before, now, item, updated):
    ...
    require(OBSERVER_ENV not in expected_environment, 'observer assignment already present in old unit')
    expected_environment[OBSERVER_ENV] = '1'
    require(environment_settings(now['main']['properties']) == expected_environment,
            'effective environment changed beyond observer assignment')
...
def prepare(args):
    ...
    require(before['process']['observer_environment'] is None,
            'old process already has observer environment; inspect before preparing')
    require(OBSERVER_ENV not in environment_settings(before['main']['properties']),
            'old unit already has observer environment')
    ...
    progress('prepare: complete native rawdb, sync/downloader and commitment-domain tests')
    run([go, 'test', '-p', '2', '-tags', 'sapling', './core/rawdb', './core/rawdb/pebbledb', './net/...', './core/state/domains',
         './core/blockbuffer', '-count=1', '-timeout=300s'],
        cwd=SOURCE, env=env, timeout=1800, log='native-affected-tests.log')
    progress('prepare: native pebbledb and blockbuffer tests with both observers enabled')
    ...
    test_pattern = ('Test.*(ProcessBlockPublishesBoundaryReadyAsyncVMRetry|StateDBCop|'
...
def wait_healthy(expected_exe, expected_sha, expected_argv, seconds=240):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            require(proc['observer_environment'] == ('1' if expected_exe == str(BINARY) else None),
                    'DB-space observer environment differs from expected release')
            head = wallet_head()
            if first is None:
                first = head
            elif head > first:
                observer = verify_observer_runtime(proc['pid']) if expected_exe == str(BINARY) else None
...
def activation_files(record):
    ...
    require(updated.replace(str(BINARY).encode(), OLD_EXE.encode(), 1) == original,
            'candidate config differs beyond executable before observer assignment')
    require(OBSERVER_ENV.encode() not in original, 'observer text already present in active unit file')
    updated = updated.rstrip(b'\n') + b'\n\n[Service]\nEnvironment=' + OBSERVER_ENV.encode() + b'=1\n'
