...
RELEASE = Path('/data/gtron/releases/20260911-commitment-cache-observation')
...
RELEASE_KIND = 'commitment-cache-observation-git-20260911'
SCRIPT_PATH = 'scripts/deploy_commitment_cache_observation_20260911.py'
OBSERVER_ENV = 'GTRON_BASE_CACHE_REUSE_OBSERVER'
OBSERVER_METRIC_PREFIX = 'blockbuffer/base_cache/reuse/'
...
EXPECTED_FILES = {
    "core/blockbuffer/base_read_cache.go": "339586649b41ab61943bf11a04afc293dbfa18259f51aa796994b299b5dde00b",
    "core/blockbuffer/base_read_cache_reuse.go": "618c131acfaa4a2c966c6c57f63041f9e72d466796ec2a4866677af5aa5e804b",
    "core/blockbuffer/base_read_cache_reuse_bench_test.go": "ef64791c70300a8b028cda07d7f089813dfa10875e2222d8a2ae82d352ec51f1",
    "core/blockbuffer/base_read_cache_reuse_test.go": "f61400e6d69131fe6ce6db44acc255d0111dc3b8dc2355ce6261c16ee42a2b4e",
    "core/blockbuffer/commitment_cache_admission_replay_test.go": "0ac399ccab9c30f37d1d15cd62373d4bb6facc1fe6caf8ae3cb0f545a439c915",
    "core/blockbuffer/commitment_cache_snapshot_guard_test.go": "b4c200a96b7833c6780a492ca8bfceede9299bbe93625f32b13422259cde41ff",
    "scripts/deploy_sync_import_ownership_20260911.py": "d3cf77461874df3d3948a74db28c13156bc29d5050e9823e4f79ca722f09f746"
}
...
def verify_observer_runtime(pid):
    require(observer_process_environment(pid) == '1', 'observer is not enabled in running process')
    ...
    values = {name: metrics.get(OBSERVER_METRIC_PREFIX + name, {}).get('value')
...
def verify_after_configuration(before, now, item, updated):
    ...
    require(now['process']['observer_environment'] == '1', 'running observer environment differs')
...
def prepare(args):
    ...
    progress('prepare: complete native sync/downloader and commitment-domain tests')
    run([go, 'test', '-p', '2', '-tags', 'sapling', './net/...', './core/state/domains',
         './core/blockbuffer', '-count=1', '-timeout=300s'],
        cwd=SOURCE, env=env, timeout=1800, log='native-affected-tests.log')
    progress('prepare: full blockbuffer tests with observer enabled')
    observer_env = dict(env)
    observer_env[OBSERVER_ENV] = '1'
    run([go, 'test', '-p', '2', '-tags', 'sapling', './core/blockbuffer',
