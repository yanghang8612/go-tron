...
func (h *handler) handleCall(cp *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	// NOTE(go-tron): the service/method names returned here were only consumed by the
	// removed OpenTelemetry RPCInfo span; discard them. See docs/dev/rpc-vendor.md.
	...
	// Upstream wrapped this section in spans via the go-ethereum internal/telemetry
	// package, which a foreign module may not import. The span bookkeeping was pure
	// instrumentation, so it is replaced with the underlying plain calls. See
	// docs/dev/rpc-vendor.md.
...
//
// NOTE(go-tron): the upstream signature took a variadic attributes ...telemetry.Attribute
// and wrapped the response encoding in an OpenTelemetry span. Both were dropped during
// vendoring because the go-ethereum internal/telemetry package is not importable from a
// foreign module; all call sites passed no attributes. See docs/dev/rpc-vendor.md.
