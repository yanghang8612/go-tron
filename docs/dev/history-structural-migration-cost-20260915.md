...
本文自身不执行删除、停机、迁移或重启。此前线上修复保留原物理格式；本次已部署新引用
容器 writer，但旧冷文件可以继续被读取，**不要求全部转换后才能使用新 writer**。

当前可以给出有效历史存量和逐批空间预算，**不能给出可信的全仓迁移或创世重同步 ETA**。
新格式双读、有界 writer、逐 trio 迁移事务及恢复测试已实现。修正版本的同输入原生
完整构建中位耗时减少 **19.7385%**，输出却增加 **6.4760%**，内容等价；该结果支持
进入有界生产验证，不能表述为体积改善或全仓迁移性能已通过。首个 trio 就地迁移和生产
切换已成功：命令耗时 **92.8436 秒**，该 trio 从 222,407,358 B 增至 376,210,416 B，
**增加 69.1538%**，三个旧文件已删除。随后完整 10 分钟线上窗口仅净减积压 267 块，
属于基本持平，未证明持续积压已经彻底解决。**暂停主动全量旧历史迁移**；保留混合格式
读取，新热范围继续使用新 writer。若以后转换选定旧段，仍由唯一发布者逐 trio 改 manifest。
...
- 下述已完成线上窗口的源码为 `197b73a920a826f21b460b3a85cc0e7ba1727d99`。06:33:33–06:45:33 UTC
...
修正版本的原生准备与 ABBA 依据分别为
[`prepare-observed.json`](../../build/benchmarks/20260915-history-reference/native-v2/prepare-observed.json)
和 [`abba-observed.json`](../../build/benchmarks/20260915-history-reference/native-v2/abba-observed.json)。
它们同样是原生终端观察摘要，完整原始产物仍在服务器；本地摘要不冒充已下载原件。
首段迁移和目录观测来自
[`migration-observed.json`](../../build/benchmarks/20260915-history-reference/native-v2/migration-observed.json)、
[`disk-observed.json`](../../build/benchmarks/20260915-history-reference/native-v2/disk-observed.json)；
完整在线采样及口径见[本次上线报告](history-reference-rollout-20260915.md)。
...
现已完成下文所列一个生产 trio 的转换测量，仍不能把完整迁移成本写成 `O(unique bytes)`；旧格式
...
源码审计范围为当前仓库；新迁移事务已在临时目录完成本地 normal/race 测试，
并完成下文所列首个生产 trio 的就地迁移。它不代表所有旧格式/大小层都已测量：
...
不是旧冷文件转换或线上整周期维护对照。它支持“首版未通过性能门槛且输出增加”，不支持
“新结构已更快”，也不能用这个 7% 预测 657.511 GB 全仓的新输出量。

首版遗漏了将临时 V2 tx index 重写为原 V7 的步骤，后续已补齐；writer 的 128 KiB
目录页缓存及单遍 R4 源流水线也已进入修正版本。首版数值保留为历史结果，不能替代
下述修正版本复测。

修正版本源码 `92767f83c54288210ec576cb7a61c1c972653b0a` 与操作脚本版本
`e3fcdafe2d4eaab73e16aecf7c47a8c71d03e2ca` 已推送。原生准备退出码为 0，release 为
`/data/gtron/releases/20260915-history-reference-v2`；准备记录绑定 2,089 个源码文件，
binary SHA-256 为 `0a9b878269fce399c6dc0a6c683908d6a4d0c323d2e6326b214216808f727e2f`，
prepared SHA-256 为 `ec0fcdd45cb989195f73984d57f7e1e2bc0323a705e7f755be933c80bb49ea00`。
准备记录本身只证明该二进制身份与构建结果；后续生产切换由单独的迁移/激活记录证明。

08:52:10–08:53:34 UTC，同一原生二进制、同一份固定 16 块输入，两个模式均使用 R4/shared chunk cache，旧模式
使用 CDC1；每个模式记录 6 次完整构建，ABBA 结果为：

| 完整构建指标 | 原路径 | 修正 R1 | 变化 |
| --- | ---: | ---: | ---: |
| wall seconds 中位数 | 0.6648537875 | 0.533621421 | **减少 19.7385%** |
| 完整 trio 文件长度 | 12,421,391 B | 13,225,799 B | **增加 6.4760%** |

`complete`、`input_unchanged`、`full_digests_equal` 均为 true。该固定输入的完整构建
速度改善已得到原生证据，输出仍更大；不能将其称为压缩率改善，也不能把增加 6.4760%
直接乘以全仓 657.511 GB 预测迁移后的占用。热输入新建与既有冷文件转存保留的引用边界
不同，必须另测冷文件转换的实际输出率。

该结果支持开始有界生产验证，尚不证明包含发布、裁剪、合并、GC 和恢复等待的完整维护
周期改善，更不是全仓迁移 ETA。现已完成首段迁移及下述线上窗口；两者必须分别报告，
不能把原生 19.7385% 当作线上同高度提升。

## 首个就地转换的实测成本与后续状态

08:56:25.864–08:57:58.708 UTC，原库 txNum **1–1,659,775** 的一个 trio 转换成功，
退出码 0、`journal_pending=false`、`database_copied=false`。当次共 3,514 个 trios，
完成 1 个、其余 3,513 个；这些计数不与早先库存拼成同一 manifest。

| 指标 | 实测值 |
| --- | ---: |
| 迁移函数内部耗时 / 整条命令耗时 | 91.508877 秒 / 92.843555 秒 |
| 包含切换操作的总区间 | 122.089401 秒；不是精确的服务不可用时长 |
| 源 / 新 trio 文件长度 | 222,407,358 B / 376,210,416 B |
| 源 / 新 history 文件长度 | 148,229,628 B / 302,032,686 B |
| accessor / index | 65,690,630 B / 8,487,100 B，均为 V7，保持原字节 |
| history 逻辑量 / 记录数 | 776,435,137 B / 6,707,959 条 |
| 新容器 chunks / spans | 5,924 / 5,924 |
| 已删除旧文件 / 长度 | 3 个 / 222,407,358 B |
| 入场工作空间估计 | 3,700,695,174 B；估计值，不是实测峰值 |

新 active 三件套总长度从 663,793,424,387 B 增至 663,947,227,445 B，精确净增
153,803,058 B，与这一个 trio 的增量一致。删除旧文件并不等于净节省空间：新 history
更大，该 trio 净增长 69.1538%。同卷 free 从 2,072,127,959,040 B 变为
2,071,947,567,104 B；这是前后可用空间差，不是临时峰值，也不能全归因于同一个阶段。
该早期范围有 670 万条记录；不能用它的 92.84 秒乘全部 trio 数估算全仓停机时间，
也不能把其 69.15% 膨胀率外推到所有 CDC/V2 冷文件。

转换代码将旧 Zstd 页解码后，由 R1 writer 统一写为 Snappy/raw；两个 companion 保持
原字节，增长集中在 history payload。该段 R1 的 chunk/span 目录共 568,704 B，远不足
以解释 153.8 MB 的增量。压缩修复还须覆盖混合格式合并，因为它也会通过当前转码和
writer 路径改写旧输入。停止主动全量迁移不意味着生产合并完全不会遇到旧格式。

切换后进程 PID **4378**，start identity `1789462679310623368`。09:05:27–09:15:27 UTC
21 点全部成功且同进程；按请求开始时间差 599.956161 秒计算，head 约 15.541 块/秒，
cold published 约 15.201 块/秒，派生 eligible 约 14.756 块/秒，lag
955,497→955,230，仅减少 267。该窗基本持平，并非已经形成足够快的持续追赶。
窗口内未完成新的 snapshot compaction merge，不能据此宣称完整合并周期也已通过。
Pebble `disk/size` 增加 158,869,272 B，LSM compaction debt 增加 2,006,422,327 B；
两者含义不同，debt 不应加到文件大小或当作历史 lag。

09:13:50 UTC 的运行中顺序 `du` 为 chaindata 332,153,798,656 B、snapshot 目录
773,855,272,960 B、ancient 247,465,566,208 B，可用空间 2,071,173,222,400 B。
同次 manifest generation 44,300 有 47 个 R1 history / 3,545 个 history；这是新写入与
已存在格式的库存，不意味着主动迁移了 47 个旧段，也不是完整全仓内容认证。

因此暂停主动全量旧历史迁移。后续方向是评估 **Zstd 保帧转存**与**当次完整语义证明的
安全复用**，在保持完整认证、输入身份和失败处理的前提下降低重复成本；两者尚未实现
或验证，不计入当前空间和时间收益。旧冷文件继续兼容读取，新 writer 无需等待全量迁移。
...
3. **冷迁移测完整 trio。** 首个原库有界 trio 已完成，主动全量迁移暂停；后续按诊断
   需要补不同规模/容器层的固定输入测量。必要输出只占该批额外空间，不复制数据库。同输入交替测旧逐行转码
...
工具已落实，修正版本的固定热输入原生构建性能门槛已通过；原库首个 trio 的有界迁移
已完成但体积显著增加，线上窗口基本持平。保留新 writer，暂停主动全量旧历史转换，
继续取得 1–4 的分层证据再判断后续批量成本。无需先清空，也不要求整仓
备份。源码与输入/结果 identity、每批 manifest 提交和错误记录仍须可复核，便于定位问题。
如果迁移失败使原仓不可用，或已经证明重放更合适，可走用户接受的清空重同步路线。
目前已取得一个 manifest 时点的全仓声明逻辑量，仍缺完整内容认证及各层新格式输出率、
临时峰值、迁移端到端速度和分层执行速度；
