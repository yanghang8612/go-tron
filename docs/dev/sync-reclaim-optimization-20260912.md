...
本文件保留 **2026-09-12 本地验证阶段** 的结论和当时部署阻塞，不代表 09-13 当前状态。
09-13 已完成原生部署和完整前后测，新 PID 为 `20101`；新 range 路径本窗全部锁忙回退，
尚未观测到实际收益，详见 [2026-09-13 实际部署记录](sync-reclaim-rollout-20260913.md)。

本轮两项候选已通过最终本地测试：把符合原冷覆盖删除条件的连续 changeset 合并为粗范围，
以及让 posting 清理取得链锁后再占用共享维护 gate。当时尚未部署，也没有本轮线上后测。
当时原生 Chrome 停在 Mac 锁屏，无法重新核对服务器 PID/start ticks、unit 与二进制；
该阶段部署 helper 的现场身份未冻结并拒绝 prepare。源码已提交 master：
