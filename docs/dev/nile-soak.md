...
be integer evidence; fractional values are rejected. When
`--require-sample-prometheus-artifact` is used with archive API evidence, the
Prometheus artifact must also expose per-method success gauges for the default
archive method set, using `0` for methods that were requested but did not
succeed, so scrape-based gates cannot miss absent probes.
