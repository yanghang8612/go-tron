...
据此修正有界 busy 调度，部署第二轮实测。新 PID 的窗口单列，检查连续冷化/裁剪速度是否高于同期导入，并重跑原历史 canary；不跨重启混算 CPU/IO 计数。
...
代码提交 `876480f8211841668473f7301f470d9aab2f6b5b` 已推送 GitHub 并在服务器 fast-forward pull。服务器独立归档构建，`CGO_ENABLED=1 -tags sapling` 的 `./core/state/... ./cmd/gtron` 七个包全部通过，原生构建 exit 0。新二进制 SHA256 为 `b10dadbdb74490ec5f3bf8f0fe32f695a5a3f7e44b11bda11b490d366d872863`，release 位于 `/data/gtron/releases/20260907-history-throughput-876480f8/`，保存同样的源码、构建与哈希信息。

原 PID 4108 于 13:34:20 UTC clean shutdown，head=1,842,922，服务退出码 0。仅替换最后一份主网 drop-in 的 release 路径并增加 `--history.catchup-mode=throughput`，原配置备份至 `deployment-backup/zz-genesis-before-throughput.conf`。**13:34:59 UTC** 新 PID **13608** 启动，`process/start/unix_nano=1788788099969088273`；日志确认 mode=throughput，`/proc/13608/exe` 与原定全部端口一致。沿原数据库恢复，没有再次清库。Nile、自动部署 service/timer 仍 inactive，原全局维护锁及主网空间保护继续保持。

13:37:12.744 UTC 再次以原热基线执行 canary，published=pruned=167,500；2043/2044/2045 的区块哈希和精确余额全部一致。启动初期因 pruned gauge 尚为零的一次 not-ready 尝试明确不计入通过。第二轮五分钟 metrics 窗口从 13:37:03 UTC 开始，首点已完成 18 个强制维护批次，published=pruned=167,500；更早含 pruned=0 的启动窗口单独保存，不用于裁剪速度结论。

**稳定窗口结果：13:37:03.230–13:42:03.235 UTC，21 点、300.056 秒。** 本地保存 [汇总](../../build/benchmarks/20260907-genesis-sync/cold-window-20260907T133703Z/summary.json)、逐点 observations 与 21 份原始 metrics，共 213,116 B。独立审读逐项核对 525 个关键字段与原始回应一致，进程标识精确一致，采集无错误或缺项。

| 同一进程五分钟窗口 | 起点 → 终点 / 实测值 |
| --- | ---: |
| head | 1,875,204 → 2,018,497 |
| eligible cutoff | 1,809,190 → 1,953,078 |
| 冷发布、热裁剪前沿 | 167,500 → 522,500 |
| 导入速度 | 477.555 blocks/s |
| 冷发布、热裁剪速度 | 1,183.113 blocks/s |
| 未冷化积压 | 1,641,690 → 1,430,578 blocks |
| 净追赶速度 | +703.576 blocks/s |
| 本窗完成的强制批次 | 71 × 5,000 blocks |
| cold / prune / lifecycle 错误 | 全部采样点均为 0 |

所有采样点的 published 与 pruned 相等，前沿无回退。相较原先约 41 blocks/s 的繁忙处理，新的五分钟平均达到约 1,183 blocks/s，且积压实际减少 **211,112 块**；比较覆盖不同区间与条件，不能将约 28.6 倍的观测比值当成同输入基准加速比。

采样到的最近一轮完整维护耗时为 505.297–916.854 ms，恢复为 3–3.667 s，这只是 21 点抽样范围，非全部 71 轮的分布。期间出现两次资源准入推迟及一次 33.398 秒完成间隔，随后恢复；因此结论是整个五分钟窗口净消减积压，不能声称每轮固定三秒或所有短窗都追得上导入。保留共享重任务锁仍可能让位于其他维护工作。

服务器 `samples-0002.jsonl` 的相邻资源窗口为 13:37:03.285–13:42:03.299 UTC，21 点、300.014 秒；汇总保存在 `/var/tmp/gtron-genesis-20260907/summary-0002-stable.json`。进程平均使用 **4.350 CPU 核**，采样 RSS 峰值 **5,687.695 MiB**；进程 IO 增量读 **958,406,656 B**、写 **20,154,515,456 B**，整盘读 **19,676,626,944 B**、写 **21,605,588,992 B**，平均队列 **1.511**、busy fraction **0.501**。共享盘净占用增加 **1,002,967,040 B**（约 0.934 GiB），不是物理空间净回收。两个采集路径的前沿读取时刻并不原子一致，CPU/IO 与上述直接 HTTP 指标各自按本窗口计算，不强行对齐瞬时水位。

新窗口导入速度低于原首轮三十分钟均值，原版本停机前的两个日志窗口已经约为 502/517 blocks/s，新进程日志也出现历史 peers 不可用提示。区块内容、网络连接、缓存和维护负荷均在变化，没有同区间重放对照，**不能据此声称导入没有退步，也不能将全部变化归因于冷化调度**。本轮证明的是：在记录的同步与资源负荷下，冷化和裁剪整体快于新增历史，积压转为下降。

验收结束时主网继续运行；空间检查通过，独立 timer active/waiting、最近 guard service Result=success。此时共享盘可用约 2.75 TiB，384 GiB 停止保留线继续有效。采样计数 CDC 输入仍为零，尚未实测后期巨大委托记录；本轮不承诺全链最终容量或固定完成时间。
