...
| Latest files | Binary `.seg` plus `.lidx` and `.bt`; new values use per-value Snappy compression when smaller, controlled by `--snapshot.compress-latest`. | Strong for point lookup and prefix iteration. Not recsplit, intentionally. |
