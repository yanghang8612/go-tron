...
- Received sync block bodies are now written to a `sync-staged-block-v1-`
  ...
  through `core/rawdb`. `SyncBodiesReady` refresh is also a downloader helper
  now: it recomputes the contiguous staged-body frontier and writes or deletes
  the ready row while reporting read/write errors back to `SyncService`.
