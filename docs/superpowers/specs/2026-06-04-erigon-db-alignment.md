...
- `gtron snapshot publish-catalog` signs the local production manifest, and
  ...
  non-empty freezer tables, preflights signed chain-freezer segment
...
- `gtron snapshot fetch` downloads a signed HTTP(S) snapshot catalog, verifies
  ...
  catalog, but only after local option parsing, bounded fetch worker-count
  validation, and the remote base URL have passed the same HTTP(S)/host/query
  validation used by the downloader. The remote URL can be supplied by
  `--snapshot.url` or the `GTRON_SNAPSHOT_URL` environment variable. Trusted
  catalog keys and optional fork config hashes have matching `GTRON_SNAPSHOT_*`
  environment defaults, giving operators one default bootstrap surface without
  hard-coding unofficial production URL or key material.
...
- `scripts/dev/nile_sync_sample.sh` now emits one JSONL sample for long-running
  ...
  archive proof with `--require-archive-tx-evidence`, add block trace proof with
  `--require-archive-trace-block`, or add transaction trace proof with
  `--require-archive-trace-transaction`. The probe
  ...
  fixture consumed by `--archive-api-fixture-file`. Sampler rows now record
  `archiveApiFixtureFile`, and acceptance can require that traceability with
  `--require-archive-fixture-file`. Acceptance also
...
- Chain-freezer snapshots now also publish a `chain-freezer-accessor` sidecar:
  ...
  Derived `chain-freezer-accessor`, `chain-index`, and `event-log-index`
  builders preflight source checksums and their required payload/index invariants
  before writing sidecars, so malformed or stale source metadata cannot be
  published as valid-looking lookup indexes.
...
- TRON solidity/PBFT account routes are covered on the same restored backend:
  ...
  `getmarketpricebypair`, `getmarketorderlistbypair`, and
  `getmarketpairlist`) and gRPC
  `WalletSolidity` market methods now dispatch through `SystemMarket` history
  at the solid/PBFT bound instead of live-head market rows. Backend coverage
  writes market order/account-order/price-list/order-book rows through temporal
  `SystemMarket` history, verifies block-1/block-2 as-of reads diverge, and
  maintains an explicit materialized `MarketOrderPairList` index so
  `GetMarketPairList` is served without scanning hashed SystemMarket keys. The
  state history reader now surfaces corrupt market order/account-order/price-list
  and pair-index protobuf payloads as archive data errors. HTTP
  solidity/PBFT `listexchanges` and gRPC
  `WalletSolidity.ListExchanges` now dispatch through `SystemExchange` history
  at the bound; HTTP `/wallet/getpaginatedexchangelist` and
  `/wallet/getexchangebyid` are registered, and the
  `/walletsolidity`/`/walletpbft` paginated and by-id exchange variants plus
  gRPC `Wallet{,Solidity}.GetExchangeById` use the same bound archive path.
  The backend reads `latest_exchange_num` and
...
- `gtron db storage-alerts --json` now packages freezer, stage, and snapshot
  ...
  `derivedIndexToBlock`, `chainLookupPruneToBlock`,
  `tailPrunedThroughBlock`, `balanceTracePruneToBlock`, and
  `sectionBloomPruneToSection`) so production
...
- The storage benchmark and Nile acceptance checkers now require readable
  ...
  `chainLookupPruneToBlock`/`coldFreezerToBlock` coverage while binding
  `tailPrunedThroughBlock` to the sampled `derivedIndexToBlock`. The storage
...
- `actuator.PrefetchKeysFor(tx)` now extracts deterministic envelope-derived
  ...
  shielded transparent endpoints plus shielded proof-cache, anchor, nullifier,
  and note-count rows, market sell/cancel including metadata-derived maker
  order rows behind compatible price levels, exchange token operations including
  metadata-derived TRC10 asset rows for both exchange token legs,
  update/unfreeze-asset operations including owner-account-derived TRC10 asset
  metadata rows, account metadata operations including account-name/account-ID
...
- `rawdb.RebuildTransactionDerivedIndexesFromBlocks` rebuilds transaction
  ...
  transaction IDs omitted when their canonical block positions prove them and
  per-receipt block number/timestamp retained only in the `TransactionRet`
  header. It
...
- `rawdb.RebuildTransactionLookupFromBlocks` is the narrower recoverable
  ...
  the redundant per-tx `ti-<hash>` row and per-receipt `TransactionInfo.Id`,
  block-number, and timestamp bytes inside `tib-`; receipt-by-ID reads resolve
  through `tx-` plus the canonical block position, while the `TransactionRet`
  decoder and block-level APIs repopulate the omitted metadata.
  ...
  pre-stage releases wrote every lookup synchronously.
...
- A 20,000-record high-entropy storage-history experiment confirms that simple
  front-coding is not worthwhile on top of zstd blocks: it reduced the raw
