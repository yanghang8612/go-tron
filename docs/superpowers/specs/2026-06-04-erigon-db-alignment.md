...
- `gtron db storage-alerts` now provides a single script-friendly alert gate for
  ...
  recorded freezer repair, missing, unbound, or impossible `ChainFreezer`
  stage progress, freezer/table bound mismatches, virtual-tail invariants,
  missing, unbound, or contradictory `SnapshotChainFreezerTailPrune` proof for
  the hidden tail,
