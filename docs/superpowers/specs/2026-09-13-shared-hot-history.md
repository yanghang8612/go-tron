...
2026-09-13。状态：已原生构建并部署，线上 writer 已显式启用。业务源码 `8058e99d`，
兼容升级运维 `6d970e0b`（首轮为 `d656be3c` / `593c7de3`）；验收数据见 [部署审计](../../dev/history-shared-hot-fix-20260913.md)。
用户授权解决重复大委托名单的热历史写放大、必要时停机检查，沿用 master 和 GitHub
原生部署；保留全部历史查询。
