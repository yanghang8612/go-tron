...
**日期：** 2026-04-15
**状态：** Draft
**目标：** 把 go-tron 的 fork 激活机制从"仅按 DP 标志 `allow_*` 读"升级为 java-tron 兼容的"SR 版本位 + 时间窗 + 比率门槛"三件套；同时完成一次 actuator/VM **执行路径** fork-gate 调用点 audit，把 java-tron 每一个 `forkController.pass(...)` 调用点镜像到对应的 go-tron 路径，并处置 go-tron 独有的 3 个 TVM flag。

对应 [PLAN.md M1.3](../../../PLAN.md#m13-分叉版本位与激活门)。依赖：M1.1（DP 已对齐）。

**执行顺序**：与 M1.2 **串行**——M1.2 先合并。原因：M1.3 Task 5 audit 会触碰 `actuator/freeze_balance.go` / `actuator/unfreeze_balance.go`，而 M1.2 Task 5 也在改这两个文件，并行会必然冲突。让 M1.2 先落地，audit 时直接在"已补权重更新"的 freeze actuator 上加 gate，路径单一。
...
4. **Fork gate audit**（按场景二分）：
   - **(a) 执行路径门**（本里程碑必做）：actuator `Validate/Execute`、VM opcode/precompile、`core/receipt.go` 能量结算、`core/block_processor.go` 共识规则。这些是链上行为分歧点，补漏会改状态。
   - **(b) 提案合法性门**（延后至 M4 gRPC）：ProposalUtil 里"X 提案只有在 Y 激活后才能提"一类的预检查。go-tron 目前还没有提案创建入口（`/wallet/createproposal`），补这些属无的放矢。
   - Audit 脚本会把 java-tron 每一处 `forkController.pass(...)` 归入 (a) 或 (b)；本里程碑只补 (a) 类缺失的 go-tron 调用点；(b) 类列进 `docs/dev/fork-audit-2026-04-15.md` 的 "Deferred to M4" 段作为 backlog。
5. **3 个 TVM flag 的处置**（已 grep 确认：三者都被 `vm/precompiles.go` 或 `vm/tvm_config.go` 使用，**不可删**）：
   - `AllowTvmShieldedToken`：`vm/precompiles.go:65` shielded token precompile 组用；java-tron 的 ShieldedTRC20 走 proposal #39 `allow_shielded_trc20_transaction`——**重命名** AllowFlag 与 DP key 到 java 规范名，并把 `vm/tvm_config.go` 读 `AllowShieldedTrc20Transaction`。
   - `AllowTvmBigInteger`：`vm/tvm_config.go:44` 喂给 TVM 配置；java 无对应 proposal。**保留**，在 `docs/dev/fork-gates.md` "go-tron specific" 段登记；`ProposalParamKey` 表不含此 ID；激活方式改为"版本位 ≥ `VERSION_4_6_0`"隐式 gate（如果 VM 该语义是 v4.6 起才对齐）——版本号选择留给 Task 6 查实际 java 语义。
   - `AllowTvmSolidity058`：`vm/tvm_config.go` 加载但未发现 interpreter 真实依赖；Task 6 做一次 `vm/` 全文检查，若仍是死叶子则仅保留 flag 定义但不向 VM 传递，并在 `docs/dev/fork-gates.md` 标 "pending removal in M7 TVM cleanup"。
6. **单元测试**
...
### 4.3 IsActive 双层 —— 方法在 ForkController 上

避免 DynamicProperties ↔ ForkController 循环依赖：`IsActive` 是 `ForkController` 的方法，控制器内部持有 DP 引用。

```go
// core/forks/controller.go
func (fc *ForkController) IsActive(flag AllowFlag) bool {
    if fc.dp.Get(dynKey[flag]) == 0 { return false }     // 软开关
    req, ok := requiredVersion[flag]
    if !ok { return true }                                // 无版本门：老 flag 或单纯软开关
    return fc.passLocked(req)                             // 版本门
}
```

包级别老函数 `forks.IsActive(flag, blockNum, dp)` 改成薄 wrapper，在迁移期保留：内部从 dp / ctx 拿 ForkController。最终所有 callsite 改成 `ctx.ForkController().IsActive(forks.AllowX)`，但不要求本里程碑一次性全改——Task 4 只提供新 API，老函数先保留；Task 5 audit 过程里就地迁移被触及的 actuator。
