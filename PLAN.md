...
| M3.5 discovery 接入 production | 完成 | 2026-05-08 | — | `cmd/gtron/main.go` 在 `p2p.NewServer` 之前构造 `discover.NewService(...)` 并传 `Discovery: discSvc`；`SetOnNewPeer` 直接走 `p2pServer.AddPeer` 完成 pong→TCP 闭环。新增 `--discover.port` flag（0 → 复用 `--p2p.port`）。`scripts/system_test.sh` 77/77 全绿（本地双节点 seed→UDP→pong→dial 路径无回归）。集成测试 `TestDiscoveryWireIn` 已落地（//go:build integration，门 `JAVA_TRON_ADDR`+`JAVA_TRON_NETWORK`，断言 60s 内 PeerCount > 1，需操作员对接公网 seed 跑一次定档）。 |
