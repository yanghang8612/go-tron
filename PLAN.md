...
| 交易类型兼容收敛波次（actuator/VM/resource） | 进行中 | — | working tree | 2026-05-17 更新：逐交易类型 review 后已修复一批会影响 `TransactionInfo` / `Account` / DP byte parity 的差异，包括公共交易校验（单 contract、ret count、tx size、expiration）、费用与回滚路径（reward/fee pool、memo/multi-sign、energy/net fee routing）、TRC10 asset bandwidth、账户/资产/提案/交易所/市场/投票/质押系列 actuator 校验与索引持久化、resource head-slot 语义、TVM `BLOCKHASH` 历史窗口、VM precompile `Pair(false,payload)` 状态位。本轮 dailyBuild 又关闭两类 VM 分叉：内部 `CREATE/CREATE2` 现在写入 java-tron 等价的 Contract 账户和 SmartContract 元数据，零长度 CALL/COPY/RETURN/LOG memory range 不再扩容，修复 #7122 energy/root 偏差。最近验证：`make gtron`、`make test`、dailyBuild clean replay 已通过 #7122/#8668 并追到 Java head #9376，blockID 与 accountStateRoot 一致。本波次尚未提交，不能填 commit。 |
| Shielded/Sapling 原生证明校验 | 未完成 | — | — | 当前只完成结构、状态写入与失败 payload parity；完整 spend/output/final proof 校验、Pedersen MerkleHash 与 librustzcash 等价能力仍未接入。该项关闭前，UI 不应把“全部交易类型完全一致”显示为 100%。 |
