...
| M11.1 witness statistics + BLOCK_FILLED_SLOTS | 完成 | 2026-04-29 | — | core/state DP 加 128 字节 rolling ring + ApplyBlockToFilledSlots/CalculateFilledSlotsCount；Witness 增 LatestBlockNum/LatestSlotNum accessor；新 consensus/dpos/statistic.go 实现 ApplyBlockStatistics（producer 计数 + miss 归因 + filled-slots ring）；wired in BlockChain.applyBlock 的 ProcessBlock 后/maintenance 前。10 个新单测，28 包全绿。**已知 gap**：(a) DigestB/C 仍未含 Witness proto + DP string 类型 → 见 spec §6；(b) 出块端 LOW_PARTICIPATION 门未接；(c) WitnessProductBlockService 作弊检测未接。 |
