...
func TestArchiveQuery_DynamicPropertiesAtUsesHistory(t *testing.T) {
	...
	commitDynamicProperties := func(blk *types.Block, n int64) tcommon.Hash {
		...
		dynProps.Set("maintenance_time_interval", n*1000)
		dynProps.SetTotalEnergyCurrentLimit(n * 10)
	...
	if got := paramValue(block1.Number(), "getMaintenanceTimeInterval"); got != 1000 {
		t.Fatalf("block1 maintenance interval = %d, want 1000", got)
	}
	if got := paramValue(block1.Number(), "getTotalEnergyCurrentLimit"); got != 10 {
		t.Fatalf("block1 total energy current limit = %d, want 10", got)
	}
	if got := paramValue(block2.Number(), "getMaintenanceTimeInterval"); got != 2000 {
		t.Fatalf("block2 maintenance interval = %d, want 2000", got)
	}
	if got := paramValue(block2.Number(), "getTotalEnergyCurrentLimit"); got != 20 {
		t.Fatalf("block2 total energy current limit = %d, want 20", got)
	}
}
...
func TestArchiveQuery_ListExchangesAtUsesSystemExchangeHistory(t *testing.T) {
	...
	if len(block1Page) != 1 || string(block1Page[0].GetFirstTokenId()) != "TOKEN" || block1Page[0].GetFirstTokenBalance() != 100 {
		t.Fatalf("block1 exchange page = %+v, want V1 pre-AllowSameTokenName exchange", block1Page)
	}
	...
	if err != nil {
		t.Fatalf("ListExchangesPaginatedAt(block2): %v", err)
	}
	if len(block2Page) != 1 || block2Page[0].GetExchangeId() != 2 || block2Page[0].GetFirstTokenBalance() != 400 {
		t.Fatalf("block2 exchange page = %+v, want second V2 exchange", block2Page)
	}
}
