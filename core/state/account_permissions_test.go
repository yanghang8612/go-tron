...
func TestAccountPermissionsPersistOutsideAccountEnvelope(t *testing.T) {
	...
	for _, test := range []struct {
		...
		got, err := decodeAccountPermissionValue(value)
		if err != nil {
...
func BenchmarkDecodeAccountPermissionRow(b *testing.B) {
	for _, fixture := range []struct {
		...
		b.Run(fixture.name, func(b *testing.B) {
			value, err := encodeAccountPermission(fixture.permission)
			if err != nil {
				b.Fatal(err)
			}
...
func TestDecodeAccountPermissionRowOwnsBytes(t *testing.T) {
	...
	raw, err := encodeAccountPermission(permission)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !proto.Equal(got, permission) {
		t.Fatalf("decoded permission mismatch\n got: %v\nwant: %v", got, permission)
	}
	for index := range raw {
		raw[index] ^= 0xff
	}
	if got.PermissionName != permission.PermissionName || !bytes.Equal(got.Operations, permission.Operations) || !bytes.Equal(got.Keys[0].Address, permission.Keys[0].Address) {
		t.Fatal("decoded permission aliases the rooted-state input buffer")
	}
}
