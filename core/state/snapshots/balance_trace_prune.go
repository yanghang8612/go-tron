...
func PruneHotBalanceTraces(db ethdb.KeyValueStore, dir string, manifest *Manifest) (*PruneHotBalanceTraceResult, error) {
	...
	for _, ref := range balanceTraceRefsAscending(manifest) {
		...
		verified, err := verifyHotBalanceTraceRowsCovered(db, seg, ref)
		...
		pruned, pruneErr := pruneHotBalanceTraceRowsForSegment(db, seg, ref, result)
		closeErr := seg.Close()
		if pruneErr != nil {
			return nil, pruneErr
		}
...
func verifyHotBalanceTraceRowsCovered(db ethdb.Iteratee, seg *BalanceTraceSegment, ref SegmentRef) (balanceTraceCoverageCheck, error) {
...
func pruneHotBalanceTraceRowsForSegment(db ethdb.KeyValueWriter, seg *BalanceTraceSegment, ref SegmentRef, result *PruneHotBalanceTraceResult) (bool, error) {
	var pruned bool
