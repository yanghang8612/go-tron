...
var (
	...
	parallelTransferAdmissionSkippedBlocksCounter     = metrics.NewRegisteredCounter("core/parallel_transfer/admission/skipped_blocks", nil)
	parallelTransferAdmissionSkippedCandidatesCounter = metrics.NewRegisteredCounter("core/parallel_transfer/admission/skipped_candidates", nil)
...
func (shadow *discardShadowBlock) preexecuteSenderChainsWithRetryState(cfg discardShadowRunConfig, chains [][]discardShadowSenderChainTask, ready func(*discardShadowTaskResult) bool, forwardRaw, retainRetryState bool) *discardShadowPreexecution {
	...
	for _, workerState := range workerStates {
		...
		go func(workerState *state.StateDB) {
			defer workers.Done()
			// A block commonly contains dozens of independent one-transaction
			// sender chains. Keep one version workspace per worker and clear it
			// between jobs instead of allocating transaction-sized maps and slices
			// for every chain. The workspace never escapes this goroutine, and
			// Prepare removes every prior chain's versions before the next task.
			versions := acquireVersionedAccessShadow(len(preCfg.transactions))
			defer releaseVersionedAccessShadow(versions)
			firstChain := true
			worker := discardShadowWorker{
				state:     workerState,
				dynProps:  workerState.DynamicProperties(),
