...
// rawdb keyspace. LogicalBytes does not include Pebble compression, indexes,
// bloom filters, WAL files, obsolete tables, or LSM write amplification.
...
// InspectProgress is emitted periodically during a long full-database scan.
...
// InspectOptions controls optional scan progress reporting.
...
var inspectSingletons = []inspectKeyspace{
	...
	{name: "state-schema-version", pattern: "=state-schema-version", key: stateSchemaVersionKey},
...
var inspectPrefixes = []inspectKeyspace{
	...
	{name: "state-account-latest", pattern: "state-account-latest-v2-*", prefix: stateAccountLatestPrefix},
...
// InspectDatabase scans all live records once and groups their logical size by
// the rawdb schema. The returned keyspaces are ordered by logical bytes, largest
// first.
func InspectDatabase(db ethdb.Iteratee, opts InspectOptions) (DatabaseInspection, error) {
	...
	start := time.Now()
	nextProgress := start.Add(opts.ProgressInterval)
	...
	for it.Next() {
		...
		if opts.Progress != nil && opts.ProgressInterval > 0 && report.Rows%100000 == 0 {
			...
			if !now.Before(nextProgress) {
				opts.Progress(InspectProgress{Rows: report.Rows, LogicalBytes: report.LogicalBytes, Elapsed: now.Sub(start)})
	...
	report.Keyspaces = make([]KeyspaceStat, 0, len(stats))
...
func inspectKeyspaceFor(key []byte) inspectKeyspace {
	...
	// Bucket by the first byte so a mainnet scan does not compare every row
	// against the entire schema catalog. Prefix buckets are longest-first to
	// keep future nested namespaces correctly attributed.
