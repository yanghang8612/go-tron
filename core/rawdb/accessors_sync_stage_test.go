...
func TestWriteSyncImportProgressBatchRejectsDeleteWithoutProgressProof(t *testing.T) {
	base := NewMemoryDatabase()
	block := testSyncStagedBlock(2, common.Hash{0x01})
	if err := WriteSyncStagedBlock(base, block); err != nil {
		t.Fatalf("write staged block: %v", err)
	...
	if result.ProgressError == nil || !strings.Contains(result.ProgressError.Error(), "staged deletes have no import progress proof") {
		t.Fatalf("result = %+v, want delete-without-progress proof error", result)
	}
	if result.Deleted != 0 || result.ProgressRows != 0 || len(result.DeleteErrors) != 0 {
		t.Fatalf("result = %+v, want no delete/progress writes", result)
	}
	...
	if _, ok, err := ReadSyncStagedBlock(base, block.Number()); err != nil || !ok {
		t.Fatalf("staged block after rejected delete-only write ok=%v err=%v, want present", ok, err)
	}
	if row, ok, err := ReadStageProgressRow(base, StageSyncImport); err != nil || ok {
		t.Fatalf("sync import progress after rejected delete-only write = %+v ok=%v err=%v, want absent", row, ok, err)
	}
}
...
func TestWriteSyncStagedBlockRawIDAndProgressWritesEquivalentRow(t *testing.T) {
	base := NewMemoryDatabase()
	db := &countingBatchStore{KeyValueStore: base}
	block := testSyncStagedBlock(3, common.Hash{0x02})
	raw, err := block.Marshal()
	...
	result := WriteSyncStagedBlockRawIDAndProgress(db, block.ID(), raw)
	if result.StageError != nil || result.ProgressReadError != nil || result.ProgressWriteError != nil {
		t.Fatalf("write result has error: %+v", result)
	}
	if !result.Staged || !result.ProgressWritten || result.ProgressSkipped {
...
func TestWriteSyncStagedBlockRawIDAndProgressRejectsMismatchedID(t *testing.T) {
	db := NewMemoryDatabase()
	block := testSyncStagedBlock(3, common.Hash{0x02})
	raw, err := block.Marshal()
	if err != nil {
	...
	result := WriteSyncStagedBlockRawIDAndProgress(db, wrong, raw)
	if result.StageError == nil || !strings.Contains(result.StageError.Error(), "sync staged raw block ID") {
		t.Fatalf("write mismatched ID result = %+v, want staged raw mismatch error", result)
	}
	if result.Staged || result.ProgressWritten || result.ProgressSkipped {
		t.Fatalf("write mismatched ID result = %+v, want no staged/progress side effects", result)
	}
	if _, ok, err := ReadSyncStagedBlockRaw(db, wrong.Num); err != nil || ok {
...
func TestWriteSyncStagedBlockRawAndProgressUpgradesSameHeightUnboundProgress(t *testing.T) {
	base := NewMemoryDatabase()
	block3 := testSyncStagedBlock(3, common.Hash{0x02})
	if err := WriteStageProgress(base, StageSyncBodies, block3.Number()); err != nil {
...
func TestWriteSyncStagedBlockRawAndProgressDoesNotRegressUnboundProgress(t *testing.T) {
	base := NewMemoryDatabase()
	block3 := testSyncStagedBlock(3, common.Hash{0x02})
	...
	if !result.Staged || !result.HadPreviousProgress || !result.ProgressSkipped || result.ProgressWritten {
		t.Fatalf("write result = %+v, want staged and skipped legacy higher progress", result)
	}
	if db.batches != 0 || db.directPuts != 1 {
		t.Fatalf("writes used batches=%d directPuts=%d, want one direct body put", db.batches, db.directPuts)
	}
	progress, ok, err := ReadStageProgressRow(db, StageSyncBodies)
	if err != nil || !ok || progress.BlockNum != 5 || progress.HasBlockHash {
		t.Fatalf("sync bodies progress = %+v ok=%v err=%v, want legacy unbound block5", progress, ok, err)
	}
}
