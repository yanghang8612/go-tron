...
func TestInspectDatabaseGroupsCurrentAndLegacyRows(t *testing.T) {
	...
	rows := []struct {
		...
		{executionSafetyIncidentKey, bytes.Repeat([]byte{0x01}, executionSafetyIncidentEncodedSize)},
		{executionSafetyQualifiedKey, []byte{executionSafetyQualificationVersion}},
	...
	for _, name := range []string{
		...
		"history-prune-mode",
		"execution-safety-incident",
		"execution-safety-qualified",
