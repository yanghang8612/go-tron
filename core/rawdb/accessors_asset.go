...
// WriteAssetIssue stores an AssetIssueContract keyed by tokenID, matching
// java-tron's AssetIssueV2Store.
...
// ReadAssetIssue returns the V2 AssetIssueContract for tokenID, or nil if not found.
...
// WriteAssetIssueByName stores the legacy AssetIssueContract keyed by token
// name, matching java-tron's pre-AllowSameTokenName AssetIssueStore.
func WriteAssetIssueByName(db ethdb.KeyValueWriter, name []byte, c *contractpb.AssetIssueContract) error {
	data, err := proto.Marshal(c)
	if err != nil {
		return err
	}
	return db.Put(assetLegacyKey(name), data)
}

// ReadAssetIssueByName returns the legacy name-keyed AssetIssueContract, or nil.
func ReadAssetIssueByName(db ethdb.KeyValueReader, name []byte) *contractpb.AssetIssueContract {
	data, err := db.Get(assetLegacyKey(name))
	if err != nil || len(data) == 0 {
		return nil
	}
	c := &contractpb.AssetIssueContract{}
...
// ListAllLegacyAssets iterates the legacy name-keyed asset store.
func ListAllLegacyAssets(db ethdb.Iteratee) []*contractpb.AssetIssueContract {
	it := db.NewIterator(assetLegacyPrefix, nil)
	defer it.Release()
	var result []*contractpb.AssetIssueContract
	for it.Next() {
		c := &contractpb.AssetIssueContract{}
...
// ListLegacyAssetsPaginated returns a page from the legacy name-keyed store.
func ListLegacyAssetsPaginated(db ethdb.Iteratee, offset, limit int) []*contractpb.AssetIssueContract {
	it := db.NewIterator(assetLegacyPrefix, nil)
	defer it.Release()
	var result []*contractpb.AssetIssueContract
	skipped := 0
