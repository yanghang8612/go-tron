...
const (
	...
	// windows while debt oscillated around only 1-1.5 GiB. After flush
	// coalescing reduced write amplification, a 2 GiB trigger still made the
	// second worker cycle on and off around routine 1.3-2.2 GiB debt. Keep L0's
	// conservative depth trigger, but let leveled debt reach 3 GiB before
	// borrowing the second worker from foreground sync. A 4-vCPU node can still
	// use both slots well before Pebble's write-stall conditions become relevant.
	l0CompactionConcurrency   = 10
	compactionDebtConcurrency = 3 << 30 // 3 GiB
)
...
// pick the package defaults documented on each field.
type Options struct {
	...
	// 1 GiB. The production ~220 GiB LSM uses a ~4.5x level multiplier with a
	// 512 MiB base; doubling it lowers the multiplier to ~3.8x and reduces overlap
	// rewrites while preserving the same SST format.
	LBaseMaxBytes int64
...
// metrics reporting should use for surfacing internal stats. tune carries the
...
func New(file string, cache int, handles int, namespace string, readonly bool, tune Options) (*Database, error) {
	...
	// Backfill tunables with package defaults so callers that left a field
	// at zero get the documented behavior instead of a hard error from Pebble.
	...
	maxMemTableSize := uint64((1<<31)<<(^uint(0)>>63) - 1)

	// Four memory tables are configured. The size of an individual memtable is
	// taken from the caller (go-tron deviation) rather than derived from the
	// cache; the cache stays a dedicated read cache. Having multiple smaller
	// memtables while keeping the per-table size bounded lets writes flush more
	// smoothly and limits write stalls under heavy compaction.
	memTableNumber := 4
	...
	opt := &pebble.Options{
		...
		//
		// go-tron deviation: go-ethereum hard-codes this to 2 (aggressive
		// compaction debt budget at the cost of more frequent L0 compactions).
		// Under sustained sync write load that pegs background compaction CPU;
		// we let the caller pick, defaulting to Pebble upstream's 4.
