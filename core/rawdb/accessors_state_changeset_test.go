...
func TestDeleteStateDomainChangeBlocksDeduplicatesBlockPostings(t *testing.T) {
	base := ethrawdb.NewMemoryDatabase()
	...
	if err := DeleteStateDomainChangeBlocks(db, []uint64{29}); err != nil {
		t.Fatal(err)
	}
...
func BenchmarkPlanStateDomainChangePostingDeletes(b *testing.B) {
	...
	db := ethrawdb.NewMemoryDatabase()
	owner := common.Address{common.AddressPrefixMainnet, 0x68}
	changes := make([]*StateDomainChange, rows)
	for i := range changes {
		changes[i] = &StateDomainChange{
