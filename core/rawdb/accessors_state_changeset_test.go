...
func TestUnwindStateDomainChangesRestoresFlatDomainsAndCommitmentRoot(t *testing.T) {
	db := ethrawdb.NewMemoryDatabase()
	owner := common.Address{0x41, 0x31}
	createdOwner := common.Address{0x41, 0x32}
	kvKey := []byte("flat/key")

	if err := WriteStateAccountLatest(db, owner, []byte("account-pre")); err != nil {
		t.Fatal(err)
	}
	mustWriteStateKVLatest(t, db, owner, 1, kvdomains.SystemReward, kvKey, []byte("kv-pre"))
	if err := WriteStateKVGeneration(db, owner, 1); err != nil {
		t.Fatal(err)
	}
	preRoot, err := RebuildLatestDomainCommitment(db)
	if err != nil {
		t.Fatalf("pre rebuild: %v", err)
	}

	if err := WriteStateAccountLatest(db, owner, []byte("account-post")); err != nil {
		t.Fatal(err)
	}
	if err := WriteStateAccountLatest(db, createdOwner, []byte("created")); err != nil {
		t.Fatal(err)
	}
	mustWriteStateKVLatest(t, db, owner, 1, kvdomains.SystemReward, kvKey, []byte("kv-post"))
	if err := WriteStateKVGeneration(db, owner, 2); err != nil {
		t.Fatal(err)
	}
	postRoot, err := RebuildLatestDomainCommitment(db)
	if err != nil {
		t.Fatalf("post rebuild: %v", err)
	}
	if postRoot == preRoot {
		t.Fatal("test setup did not move commitment root")
	}
	...
	changes := []*StateDomainChange{
		{
			...
			Owner:      owner,
			PrevExists: true,
			Prev:       []byte("account-pre"),
			NextExists: true,
			Next:       []byte("account-post"),
		},
		{
			BlockNum:   11,
			TxNum:      11,
		...
		},
		{
			...
			Owner:      owner,
			PrevExists: true,
			Prev:       EncodeStateKVGenerationValue(1),
			NextExists: true,
			Next:       EncodeStateKVGenerationValue(2),
		},
		{
			BlockNum:   11,
			TxNum:      11,
	...
	if err := UnwindStateDomainChanges(db, 11); err != nil {
		t.Fatalf("unwind: %v", err)
	}
	if got, ok, err := ReadStateAccountLatest(db, owner); err != nil || !ok || !bytes.Equal(got, []byte("account-pre")) {
		t.Fatalf("account latest after unwind = %q ok:%v err:%v", got, ok, err)
	}
	if _, ok, err := ReadStateAccountLatest(db, createdOwner); err != nil || ok {
		t.Fatalf("created account latest after unwind ok:%v err:%v", ok, err)
	}
	if got, ok, err := ReadStateKVLatest(db, owner, 1, kvdomains.SystemReward, kvKey); err != nil || !ok || !bytes.Equal(got, []byte("kv-pre")) {
		t.Fatalf("KV latest after unwind = %q ok:%v err:%v", got, ok, err)
	}
	if got, ok, err := ReadStateKVGeneration(db, owner); err != nil || !ok || got != 1 {
		t.Fatalf("KV generation after unwind = %d ok:%v err:%v", got, ok, err)
	}
	root, ok, err := ReadLatestDomainCommitmentRoot(db)
	if err != nil || !ok {
		t.Fatalf("commitment root after unwind ok:%v err:%v", ok, err)
	}
	if root != preRoot {
		t.Fatalf("commitment root after unwind = %x, want pre-root %x", root, preRoot)
	}
	rebuilt, err := RebuildLatestDomainCommitment(db)
	if err != nil {
		t.Fatalf("rebuild after unwind: %v", err)
	}
	if rebuilt != root {
		t.Fatalf("incremental root = %x, rebuild root = %x", root, rebuilt)
	}
}
