...
// JumpTable is the dispatch table mapping opcodes to operations. Keeping the
// metadata inline makes one opcode lookup fetch the handler, stack limits and
// energy cost from the same contiguous table instead of following a second
// pointer to a separately allocated operation.
...
func jumpTableForConfig(cfg TVMConfig) *JumpTable {
	...
	*table = sharedJumpTable
	for opcode := range table {
		opMeta := &table[opcode]
		if opMeta.execute != nil && opMeta.enabledFn != nil && !opMeta.enabledFn(cfg) {
...
func newJumpTable() JumpTable {
	...
	for opcode := range tbl {
		opMeta := &tbl[opcode]
		if opMeta.execute == nil {
			continue
		}
		returns := operationStackReturns(OpCode(opcode), opMeta)
		opMeta.maxInputStack = stackLimit + opMeta.minStack - returns
	}
