...
func (p *PbftProducer) OnBlockApplied(block *types.Block) {
	...
	for _, k := range keys {
		...
		if err != nil {
			pbftLog.Warn("PBFT block PREPREPARE build failed", "err", err)
			continue
		}
		p.dispatch(payload)
	}
...
func (p *PbftProducer) OnMaintenance(block *types.Block, newWitnesses []tcommon.Address) {
	...
	for _, k := range keys {
		...
		if err != nil {
			pbftLog.Warn("PBFT SRL PREPREPARE build failed", "err", err)
			continue
		}
		p.dispatch(payload)
	}
...
func (p *PbftProducer) EmitPrepare(parent *corepb.PBFTMessage_Raw) {
	...
	for _, k := range keys {
		...
		if err != nil {
			pbftLog.Warn("PBFT PREPARE build failed", "err", err)
			continue
		}
		p.dispatch(payload)
	}
...
func (p *PbftProducer) EmitCommit(parent *corepb.PBFTMessage_Raw) {
	...
	for _, k := range keys {
		...
		if err != nil {
			pbftLog.Warn("PBFT COMMIT build failed", "err", err)
			continue
		}
		p.dispatch(payload)
	}
