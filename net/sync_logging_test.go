...
func TestSync_BatchSummaryReportedOnInterval(t *testing.T) {
	...
	for _, k := range []string{
		...
		"commitmentUpdatesPerBlock=",
		"stateCommitNsPerUpdate=",
		"persistMsPerBlock=",
		"persistMetadataBytesPerBlock=",
		"persistMetadataBytesPerTx=",
		"remaining=",
		"peers=",
		"activePeers=",
		"inflight=",
	...
	for _, k := range []string{
		...
		"persistMetadataBytes=",
		"persistTransactionLookupRows=",
		"persistTraceAccounts=",
		"hooks=",
		"blockBuffer=",
		"requested=",
		"retryList=",
...
func TestReportSegmentInfoIsCompactOperationalStatus(t *testing.T) {
	...
	for _, field := range []string{
		...
		"vmTxsPerBlock=", "nativeTxsPerBlock=", "vmTxSharePct=", "rawEnergyPerSec=", "rawEnergyPerVMTx=",
		"billedToRawEnergyRatio=", "vmMsPerVMTx=", "vmNsPerRawEnergy=",
		...
		"stateCommitMsPerBlock=", "commitmentUpdatesPerBlock=", "stateCommitNsPerUpdate=", "persistMsPerBlock=",
		"persistMetadataBytesPerBlock=", "persistMetadataBytesPerTx=", "peers=2", "activePeers=1", "inflight=2",
...
func TestSyncImportWindowObservationSeparatesWorkAndSupply(t *testing.T) {
	s := tsync.Snapshot{
		...
		ApplyStats: core.ApplyStats{
			...
			VMTransactions:     10,
			NativeTransactions: 30,
			VMExecution:        100 * time.Millisecond,
			VMRawEnergyUsage:   4_000_000,
	...
	for name, check := range map[string]struct{ got, want float64 }{
		...
		"vm tx/s":                  {got.VMTransactionsPerSec, 5},
		"native tx/s":              {got.NativeTransactionsPerSec, 15},
		"vm tx/block":              {got.VMTransactionsPerBlock, 1},
		"native tx/block":          {got.NativeTransactionsPerBlock, 3},
		"vm tx share":              {got.VMTransactionShare, 0.25},
		"raw energy/s":             {got.RawEnergyPerSec, 2_000_000},
		"raw energy/vm tx":         {got.RawEnergyPerVMTransaction, 400_000},
		"billed/raw energy":        {got.BilledToRawEnergyRatio, 2},
		"vm ms/vm tx":              {got.VMExecutionMillisPerVMTransaction, 10},
		"vm ns/raw energy":         {got.VMExecutionNanosPerRawEnergy, 25},
		...
		"commit accounts/block":    {got.StateCommitAccountsPerBlock, 3},
		"commit kv accounts/block": {got.StateCommitKVAccountsPerBlock, 2},
		"commit kv items/block":    {got.StateCommitKVItemsPerBlock, 10},
		"storage writes/block":     {got.StateStorageWritesPerBlock, 5},
		"kv writes/block":          {got.StateKVWritesPerBlock, 8},
		"commitment updates/block": {got.CommitmentUpdatesPerBlock, 5},
		"commit ns/update":         {got.StateCommitNanosPerCommitmentUpdate, 4_000_000},
		"persist ms/block":         {got.PersistMillisPerBlock, 10},
		"metadata bytes/block":     {got.PersistMetadataBytesPerBlock, 1_000},
		"metadata bytes/tx":        {got.PersistMetadataBytesPerTx, 250},
		"metadata records/block":   {got.PersistMetadataRecordsPerBlock, 2},
		"lookup rows/block":        {got.PersistLookupRowsPerBlock, 1},
		"trace accounts/block":     {got.PersistTraceAccountsPerBlock, 0.5},
	} {
