...
// NewBufferedBlock returns a buffered sync block that owns the supplied wire
// bytes. The p2p receive path transfers its frame payload to sync when the
// block is consumed, so copying the full block here only increases allocation
...
// RawBlockBytes transfers ownership of the block's wire bytes to the sync
// buffer. raw is the exact payload received off the wire and callers must not
// mutate it after this call. Callers without it pass nil and the bytes are
// re-marshaled from the decoded block.
