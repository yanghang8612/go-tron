...
func makeConfig(ctx *cli.Context) *node.Config {
	return &node.Config{
		...
		SaveInternalTx:                 ctx.Bool(vmSaveInternalTxFlag.Name),
		SaveFeaturedInternalTx:         ctx.Bool(vmSaveFeaturedInternalTxFlag.Name),
		SaveCancelAllUnfreezeV2Details: ctx.Bool(vmSaveCancelAllUnfreezeV2DetailsFlag.Name),
	}
}
...
func stateSnapshotsDir(dataDir string) string {
	return filepath.Join(dataDir, "gtron", "state-snapshots")
}
...
func makeDevGenesis(witnessAddr tcommon.Address, fullFeatures bool, maintenanceInterval int64) *params.Genesis {
	...
	return &params.Genesis{
		Config: &params.ChainConfig{
			ChainID:                9999,
			P2PVersion:             1,
			BlockNumForEnergyLimit: params.MainnetChainConfig.BlockNumForEnergyLimit,
		},
		Timestamp: 0,
