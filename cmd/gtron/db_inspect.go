...
var (
	dbInspectJSONFlag     = &cli.BoolFlag{Name: "json", Usage: "Write the complete inspection report as JSON"}
	dbInspectTopFlag      = &cli.IntFlag{Name: "top", Usage: "Show only the N largest chaindata keyspaces in text output (0 = all)", Value: 25}
	dbInspectProgressFlag = &cli.DurationFlag{Name: "progress", Usage: "Progress reporting interval (0 disables)", Value: 5 * time.Second}
)
...
type ancientInspection struct {
	...
	V2Coverage    uint64             `json:"v2_coverage"`
...
type ancientTableStat struct {
	...
	HiddenBytes   uint64  `json:"hidden_bytes"`
	Percent       float64 `json:"percent"`
}

func dbInspectCommand() *cli.Command {
	return &cli.Command{
		Name:        "inspect",
		...
		Flags:       []cli.Flag{dataDirFlag, dbCacheFlag, dbHandlesFlag, dbInspectJSONFlag, dbInspectTopFlag, dbInspectProgressFlag},
		Action:      dbInspectCmd,
	}
}
...
func dbInspectCmd(ctx *cli.Context) error {
	...
	if info, statErr := os.Stat(ancientPath); statErr == nil && info.IsDir() {
		output.Ancient, err = inspectAncient(ancientPath)
		if err != nil {
			return fmt.Errorf("inspect ancient %q (stop gtron before inspection): %w", ancientPath, err)
		}
...
func inspectAncient(path string) (*ancientInspection, error) {
	...
	if err := filepath.WalkDir(path, func(filePath string, entry fs.DirEntry, walkErr error) error {
		...
		for table := range tableConfigs {
			...
			isV1 := strings.HasPrefix(entry.Name(), table+".")
			isV2 := relativeErr == nil &&
				strings.HasPrefix(relative, filepath.Join("v2", table)+string(filepath.Separator)) &&
				strings.HasSuffix(entry.Name(), ".gtv2")
			if isV1 || isV2 {
	...
	freezer, err := rawdbfreezer.NewFreezer(path, "", true, freezerTableSize, tableConfigs)
	if err != nil {
		return nil, err
	}
	defer freezer.Close()
	stats, err := freezer.Stats()
	if err != nil {
		return nil, err
	}

	report := &ancientInspection{Path: path, PhysicalBytes: total, Head: stats.Head, Tail: stats.Tail, V2Coverage: stats.V2Coverage}
	logicalTail := stats.Tail
	if stats.V2Coverage > 0 {
		logicalTail = 0
	}
	for _, table := range stats.Tables {
		stat := ancientTableStat{
			Name:          table.Name,
			Rows:          table.Head - logicalTail,
			PhysicalBytes: tableBytes[table.Name],
			VisibleBytes:  table.VisibleSize + table.V2Size,
			HiddenBytes:   table.HiddenSize,
		}
		if total != 0 {
...
func writeDBInspectionText(writer io.Writer, output dbInspectionOutput, top int) {
	...
	if output.Ancient != nil {
		fmt.Fprintf(writer, "\nAncient: %s\n", output.Ancient.Path)
		fmt.Fprintf(writer, "Physical files: %s; range=[%d,%d); V2 coverage=[0,%d)\n", formatIEC(output.Ancient.PhysicalBytes), output.Ancient.Tail, output.Ancient.Head, output.Ancient.V2Coverage)
		tw = tabwriter.NewWriter(writer, 0, 4, 2, ' ', 0)
		fmt.Fprintln(tw, "TABLE\tROWS\tPHYSICAL\tVISIBLE\tHIDDEN\tSHARE")
		for _, stat := range output.Ancient.Tables {
			fmt.Fprintf(tw, "%s\t%d\t%s\t%s\t%s\t%.2f%%\n", stat.Name, stat.Rows, formatIEC(stat.PhysicalBytes), formatIEC(stat.VisibleBytes), formatIEC(stat.HiddenBytes), stat.Percent)
		}
		_ = tw.Flush()
	}
	fmt.Fprintln(writer, "\nNote: chaindata logical bytes are uncompressed live key/value bytes, not per-keyspace SST disk usage. Ancient physical bytes include table indexes and metadata.")
}
