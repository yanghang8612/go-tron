...
func dbCommand() *cli.Command {
	return &cli.Command{
		...
		Subcommands: []*cli.Command{
			...
			dbHistorySharingBenchmarkCommand(),
			dbHistoryRangeExportCommand(),
			dbHistoryColdBenchmarkCommand(),
...
type dbStorageAlertsJSON struct {
	...
	SectionBloomPruneToBlock     int64                     `json:"sectionBloomPruneToBlock"`
...
type dbStoragePruneEvidence struct {
	...
	SectionBloomPruneToBlock int64
}
...
func dbStoragePruneEvidenceFromStageRows(rows []dbStageStatusRow) dbStoragePruneEvidence {
	...
	for _, row := range rows {
		...
		case rawdb.StageSnapshotSectionBloomPrune:
			evidence.SectionBloomPruneToBlock = value
		}
	}
	return evidence
}
...
func dbStorageAlertsCmd(ctx *cli.Context) error {
	...
	report := dbStorageAlertsJSON{
		...
		SectionBloomPruneToBlock:     pruneEvidence.SectionBloomPruneToBlock,
...
func dbWriteStorageAlertsPrometheus(w io.Writer, report dbStorageAlertsJSON) {
	...
	for _, boundary := range []struct {
		...
		{field: "sectionBloomPruneToBlock", value: report.SectionBloomPruneToBlock},
	} {
...
func dbStageStatusGroup(stage rawdb.StageID) string {
	...
	case rawdb.StageHeaders, rawdb.StageBodies, rawdb.StageExecution, rawdb.StageCommitment, rawdb.StageFinish:
		return "canonical"
	case rawdb.StageTxLookup, rawdb.StageStateHistoryIndex:
		return "derived"
